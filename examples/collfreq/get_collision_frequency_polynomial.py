...
def dEfunc(p, t):
        dparams = polyder(p, m=1)
        return polyval(dparams, t)
...
yvals_avg = movingaverage(yvals, avgwindow)
# cut window
xvals_avg  = xvals[    avgwindow:-avgwindow:avgwindow]
yvals_avg  = yvals_avg[avgwindow:-avgwindow:avgwindow]
dyvals_avg = diff(yvals_avg) / diff(xvals_avg)

p1 = polyfit(xvals_avg, yvals_avg, polydegree)
...
ax.plot(xvals_avg*unit_t0_in_fs, yvals_avg, 'o-', label='moving average: %s' % datanames[7], linewidth=1.5)
...
ax.plot(xvals         *unit_t0_in_fs, dyvals_fit, '-', label=r'dE / dt (fit)',        linewidth=1.5)
ax.plot(xvals_avg[:-1]*unit_t0_in_fs, dyvals_avg, 'o-', label=r'dE / dt (avg)',        linewidth=1.5)
