    ...
    subroutine load_data(index, filename)
	  ...
	  if (datindex > Nt) then
	    ...
	    exit
          ...
          read(87,'(" ",1(1x,i19),3(1x,g19.6),5(1x,i19))',iostat=ios) tmpi(1), tmpr(1:3), tmpi(2:6)
	  
	  rawdata_all(index, datindex, 1)   = tmpi(1)
	  rawdata_all(index, datindex, 2:4) = tmpr(1:3)
	  rawdata_all(index, datindex, 5:9) = tmpi(2:6)
...
program clusterdata_jackknife
  ...
  filename_in = trim(dirname) // '/cluster.dat'
  call count_num_lines(trim(filename_in), numlines)
  
  numlines=numlines-2 ! there are two header lines in cluster.dat
  
  call allocate_data(argc, numlines, 8) ! TODO: determine the parameters automatically
  ...
  call jackknife()
  call write_data('./cluster_jackknife.dat', './cluster_jackknife_stddev.dat')
  write(*,*) argc, valid
  call free_data()
