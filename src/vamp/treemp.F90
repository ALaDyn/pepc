...
!VAMPINST program_start
      CALL VTdefs()
      CALL VTENTER(IF_treemp,VTNOSCL,VTIERR)
      write(*,*) 'VT: treemp P>',VTIERR, &
         IF_treemp,ICLASSH
!
  call MPI_INIT(ierr)
...
!  if (me ==0 .and. vis_on) call flvisit_spk_init() ! Start up VISIT
  ...
  do itime = 1,nt
     ...
     if (beam_config ==2 .and. vis_on) call beam_control   ! Check for new particle source parameters
...
!VAMPINST program_end
      ...
      write(*,*) 'VT: treemp P<',VTIERR,ICLASSH
!
  stop
!VAMPINST program_end
      CALL VTLEAVE(ICLASSH,VTIERR)
