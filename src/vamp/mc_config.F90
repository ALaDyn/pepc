...
!VAMPINST subroutine_start
       CALL VTENTER(IF_mc_config,VTNOSCL,VTIERR)
!      write(*,*) 'VT: mc_config S>',VTIERR,
!     *    IF_mc_config,ICLASSH
!
  mc_dump = 10
...
!	call potenergy(epot) ! Compute new (accurate) potential energy
	...
	enddo
     ...
     if (vis_on .and. mod(j,mc_steps/10) == 0 )  call visualize
...
!      write(*,*) 'VT: mc_config S<',VTIERR,ICLASSH
!
