...
!
  ...
  namelist /pepcdata/ nep, nip, ne, ni, &
       ...
       nt, dt, mc_steps, idump, ivis, nmerge, ngx, ngy, ngz, &
...
!VAMPINST subroutine_start
       CALL VTENTER(IF_setup,VTNOSCL,VTIERR)
!      write(*,*) 'VT: setup S>',VTIERR,
!     *    IF_setup,ICLASSH
!
  tree_debug = .false.
  domain_debug = .false.
...
!  nbaddr = 15                  ! # bits for cell address in hash-table
  ...
  size_tree = max(2*nintmax+5*nppm,1000)
...
!      write(*,*) 'VT: setup S<',VTIERR,ICLASSH
!
