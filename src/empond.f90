...
subroutine empond(t,tpulse,sigma0,a0,w0,x,y,z,ez,by,az,phipon)
  ...
  real, intent(out) :: phipon, ez, by, az ! pond. potential and fields

  real :: Tpon, Rpon, gamma, sigma, atten, theta
  real :: r, pi=3.141592654, phi, chi, phase
  real :: k0, lskin, gamma_c, ezhelm, byhelm, wp_r, nonc
  ...
  if (x.le.0) then
     ...
     ezhelm = sin(chi)     ! laser 'Ez'
     byhelm = cos(chi)     ! laser 'By'
     sigma = sigma0*sqrt(1.+abs(x)**2/10./sigma0**2) ! take Rayleigh length 4*sigma0

  else
     !   evanescent wave - need Sudan solution here
     ezhelm = sin(phi)*exp(-x/lskin)        ! laser Ez inside
     byhelm = cos(phi)*exp(-x/lskin) ! laser By inside
  ...
  ez = 2*a0*ezhelm*cos(phase)*tpon*Rpon 
  by = 2*a0*byhelm*sin(phase)*tpon*Rpon
  az = -2*a0*ezhelm*sin(phase)*tpon*Rpon

  phipon = 4*a0**2*ezhelm**2*Rpon**2
