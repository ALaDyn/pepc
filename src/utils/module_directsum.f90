         ...
         do j = 1, nreceived
            do i = 1, size(particles)
               ...
               do ibox = 1, num_neighbour_boxes ! sum over all boxes within ws=1
                  ...
                  if (all(abs(delta) .lt. spatial_interaction_cutoff)) then
#endif
                     if (currank .eq. 0) then
                        if ((ibox .eq. num_neighbour_boxes) .and. (testidx(j) .eq. i)) then
...
!$OMP END PARALLEL DO
      ...
      DEBUG_ASSERT(ntest .eq. nreceived)
      DEBUG_ASSERT(size(directresults, kind=kind_particle) .ge. ntest)
