module module_accelerator
   ...
   function acc_loop() bind(c)
      ...
      do while (atomic_load_int(acc%thread_status) .ne. ACC_THREAD_STATUS_STOPPED)
         ...
         do while (atomic_load_int(acc%q_top) .ne. atomic_load_int(acc%q_bottom))
            ...
            if (acc%acc_queue(tmp_top)%entry_valid) then
               ...
               ! store results in original data
               !!acc%acc_queue(tmp_top)%results = tmp_particle%results
               !!acc%acc_queue(tmp_top)%particle%work    = tmp_particle%work

               ! free lists
               deallocate(acc%acc_queue(tmp_top)%partner)
               nullify(acc%acc_queue(tmp_top)%partner)

               ! we have to invalidate this request queue entry. this shows that we actually processed it and prevents it from accidentially being resent after the queue wrapped around
      ...
      ! just doodle for a while...
      call system('sleep 2s')
   ...
   subroutine dispatch_list(particle, eps2, penalty)
      ...
      integer :: local_queue_bottom, gpu_status
      ...
      write(*,*) 'dispatching ',local_queue_bottom
      ...
      !!acc%acc_queue(local_queue_bottom)%results => particle%results
