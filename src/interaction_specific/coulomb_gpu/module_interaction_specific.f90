...
contains
   ...
   !>
   !> 'barrier' to flush all possible internal accelerator caches and make sure they are finished
   !>
...
!      real(kind_physics), intent(inout) :: dist2
!      real(kind_physics), intent(inout) :: boxlength2
!
...
!   end function
   ...
   subroutine calc_force_per_interaction_global(particle, node, node_idx, delta, dist2, vbox, node_is_leaf)
      ...
      real(kind_physics):: exyz(1:3), phic
   ...
   contains
      ...
      subroutine add_to_iact_list(particle, node, delta, node_is_leaf)
         ...
         type(t_particle_thread), intent(inout) :: particle
         type(t_tree_node_interaction_data), intent(in) :: node
         real(kind_physics), intent(in) :: delta(3)
         logical, intent(in) :: node_is_leaf
   ...
   subroutine compute_iact_list(particle)
      ...
      integer :: idx
      real(kind_physics) :: exyz(1:3), phic
      real(kind_physics) :: delta(1:3), dist2
...
!         write(*,*) 'calling dispatcher'
   ...
   subroutine calc_force_per_particle(particles)
      ...
      if (do_periodic .and. include_far_field_if_periodic) then
         ...
         !$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(phi_lattice, e_lattice) SHARED(particles) SCHEDULE(RUNTIME) REDUCTION(+:potfarfield,potnearfield) NUM_THREADS(num_threads)
