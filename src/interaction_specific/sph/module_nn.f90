...
contains
   ...
   subroutine nn_prepare_particleresults(t, particles)
      ...
      do i = 1, size(particles)
         ...
         do
            if (t%nodes(node)%leaves .ge. num_neighbour_particles) then
            ...
            DEBUG_ASSERT_MSG(node .ne. NODE_INVALID, *, 'Reached invalid node: Some nodes %parent is not set correctly or we reached the root node')
         end do
