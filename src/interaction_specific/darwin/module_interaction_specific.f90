     ...
     private
      ...
      subroutine multipole_from_particle(particle_pos, particle, multipole)
        ...
        multipole = t_tree_node_interaction_data(particle_pos, &            !position
                                     ...
                                     (/0., 0., 0./), &                   !dipole
                                     (/0., 0., 0./), &          !diagonal quadrupole
                                       0., 0., 0., &               !mixed quadrupole
                                     (/q*vx, q*vy, q*vz/), &      !current density monopole
                                     (/0., 0., 0./), &    !current density dipole - qj*vxj*x - qj*vxj*y - qj*vxj*z
                                     (/0., 0., 0./), &    !current density dipole - qj*vyj*x - qj*vyj*y - qj*vyj*z
                                     (/0., 0., 0./), &    !current density dipole - qj*vzj*x - qj*vzj*y - qj*vzj*z
                                     (/0., 0., 0./), & !current density diagonal quadrupole - qj*vxj*x**2 - qj*vxj*y**2 - qj*vxj*z**2
                                     (/0., 0., 0./), & !current density diagonal quadrupole - qj*vyj*x**2 - qj*vyj*y**2 - qj*vyj*z**2
                                     (/0., 0., 0./), & !current density diagonal quadrupole - qj*vzj*x**2 - qj*vzj*y**2 - qj*vzj*z**2
                                     (/0., 0., 0./), &  !current density mixed quadrupole
                                     (/0., 0., 0./), &  !current density mixed quadrupole
                                     (/0., 0., 0./), 0. ) !current density mixed quadrupole / bmax
      ...
      subroutine shift_multipoles_up(parent, children)
        ...
        parent%quadjy = [0., 0., 0.]
        parent%quadjz = [0., 0., 0.]
        parent%quadjxy= [0., 0., 0.]
        parent%quadjyz= [0., 0., 0.]
        parent%quadjzx= [0., 0., 0.]
      ...
      subroutine calc_force_per_interaction_with_leaf(particle, node, node_idx, delta, dist2, vbox)
        ...
        real(kind_physics) :: exyz(3), Bxyz(3), Jxyz(3), Jirrxyz(3), phic, rhoc, Axyz(3)
        ...
        select case (force_law)
          ...
          case (6)  !  compute 2D-Coulomb/Darwin fields and potential
              ! It's a leaf, do direct summation with calc_force_darwin_2D_direct
              call calc_force_darwin_2D_direct(node, delta(1:2), dot_product(delta(1:2), delta(1:2)) + eps2, eps2, exyz(1:2), Axyz(1:2), Jxyz(1:2), Jirrxyz(1:2), Bxyz(3), phic)
              ...
              particle%results%e(1:2)       = particle%results%e(1:2)    + exyz(1:2)
              particle%results%pot          = particle%results%pot       + phic

          case (7)  !  compute 3D-Coulomb/Darwin fields and potential
              ! It's a leaf, do direct summation with calc_force_darwin_3D_direct
              call calc_force_darwin_3D_direct(node, delta(1:3), dot_product(delta(1:3), delta(1:3)) + eps2, eps2, exyz(1:3), Axyz(1:3), Jxyz(1:3),  Jirrxyz(1:3) , Bxyz(1:3), phic)
              ...
              particle%results%Jirr      = particle%results%Jirr + Jirrxyz
              particle%results%e         = particle%results%e    + exyz
              particle%results%pot       = particle%results%pot  + phic
...
!
!        if ( (force_law .eq . 6).or.(force_law .eq . 7) ) then
!            particle%results%A(1:3)         = particle%results%A(1:3)    + Axyz(1:3)
!            particle%results%B(1:3)         = particle%results%B(1:3)    + Bxyz(1:3)
!            particle%results%J(1:3)         = particle%results%J(1:3)    + Jxyz(1:3)
      ...
      subroutine calc_force_per_interaction_with_twig(particle, node, node_idx, delta, dist2, vbox)
        ...
        select case (force_law)
          ...
          case (6)  !  compute 2D-Coulomb/Darwin fields and potential
              ...
              call calc_force_darwin_2D(node, delta(1:2), dot_product(delta(1:2), delta(1:2)) + eps2, eps2, exyz(1:2), Axyz(1:2), Jxyz(1:2), Jirrxyz(1:2), Bxyz(3), phic)
              particle%results%A(1:2)       = particle%results%A(1:2)   ! + Axyz(1:2)
              particle%results%B(3)         = particle%results%B(3)     ! + Bxyz(3)
              particle%results%J(1:2)       = particle%results%J(1:2)    + Jxyz(1:2)
              particle%results%Jirr(1:2)    = particle%results%Jirr(1:2) + Jirrxyz(1:2)
          ...
          case (7)  !  compute 3D-Coulomb/Darwin fields and potential
              call calc_force_darwin_3D(node, delta(1:3), dot_product(delta(1:3), delta(1:3)) + eps2, eps2, exyz(1:3), Axyz(1:3), Jxyz(1:3), Jirrxyz(1:3), Bxyz(1:3), phic)
              particle%results%A(1:3)         = particle%results%A(1:3)    + Axyz(1:3)
              particle%results%B(1:3)         = particle%results%B(1:3)    + Bxyz(1:3)
              particle%results%J(1:3)         = particle%results%J(1:3)    + Jxyz(1:3)
              particle%results%Jirr(1:3)      = particle%results%Jirr(1:3) + Jirrxyz(1:3)
              particle%results%e(1:3)         = particle%results%e(1:3)    + exyz(1:3)
              particle%results%pot            = particle%results%pot       + phic
