...
!      real(kind_physics), parameter :: three= 3._kind_physics
   ...
   complex(kind_physics) :: mu_cent(1:fmm_array_length_taylor)
   complex(kind_physics) :: omega_tilde(1:fmm_array_length_multipole)
   complex(kind_physics) :: MLattice(1:fmm_array_length_taylor)
...
!          use module_mirror_boxes, only : mirror_box_layers,t_lattice_1,t_lattice_2,t_lattice_3,LatticeCenter,LatticeOrigin
      implicit none
      integer(kind_pe), intent(in) :: mpi_rank
      integer, intent(in) :: mpi_comm
      real(kind_particle) :: left(1:2), right(1:2)
   ...
   subroutine calc_lattice_coefficients(ML)
      ...
      complex(kind_physics), intent(out) :: ML(1:fmm_array_length_taylor)
      ! Variables for lattice coefficient calculation
      complex(kind_physics) :: Mstar(1:fmm_array_length_multipole)
      complex(kind_physics) :: Lstar(1:fmm_array_length_taylor)
   ...
   subroutine zero_terms_taylor(M)
      ...
      implicit none
      complex(kind_physics), intent(inout) :: M(1:fmm_array_length_taylor)

      ! 1D, 2D, and 3D periodicity
      M(tblinv(0, 0, Lmax_taylor)) = (zero, zero)

      if (count(periodicity) .gt. 2) then
   ...
   subroutine zero_terms_multipole(O)
      use module_mirror_boxes, only: periodicity
      implicit none
      complex(kind_physics), intent(inout) :: O(1:fmm_array_length_multipole)
...
!$    call omp_set_num_threads(1)
      ...
      if (fmm_extrinsic_correction .eq. FMM_EXTRINSIC_CORRECTION_FICTCHARGE) then
      ...
      call MPI_ALLREDUCE(MPI_IN_PLACE, omega_tilde, 2 * fmm_array_length_multipole, MPI_KIND_PHYSICS, MPI_SUM, MPI_COMM_fmm, ierr)

      if ((myrank .eq. 0) .and. dbg(DBG_PERIODIC)) then
         call WriteTableToFile('omega_tilde.tab', omega_tilde, Lmax_multipole)
      end if

      if (abs(omega_tilde(tblinv(0, 0, Lmax_multipole))) .gt. 0.) then
   ...
   contains
      subroutine addparticle(om, R, q)
         implicit none
         complex(kind_physics), intent(inout) :: om(1:fmm_array_length_multipole)
         real(kind_physics), intent(in) :: R(3)
         real(kind_physics), intent(in) :: q
   ...
   subroutine calc_box_dipole(particles)
      ...
      ! sum contributions from all processors
      call MPI_ALLREDUCE(MPI_IN_PLACE, box_dipole, 3, MPI_KIND_PHYSICS, MPI_SUM, MPI_COMM_fmm, ierr)
      call MPI_ALLREDUCE(MPI_IN_PLACE, quad_trace, 1, MPI_KIND_PHYSICS, MPI_SUM, MPI_COMM_fmm, ierr)
   ...
   subroutine calc_mu_cent(omega, mu)
      implicit none
      complex(kind_physics), intent(in) :: omega(1:fmm_array_length_multipole)
      complex(kind_physics), intent(out) :: mu(1:fmm_array_length_taylor)
   ...
   !>
   recursive real(kind_physics) function Ptilda(l, m, x) result(Pt)
      implicit none
   ...
   !>  http://mathworld.wolfram.com/AssociatedLegendrePolynomial.html
   !>
   recursive real(kind_physics) function P2tilda(l, m, x) result(P2t)
      implicit none
      integer, intent(in) :: l, m
      real(kind_physics), intent(in) :: x
   ...
   !>
   complex(kind_physics) function OMultipole(l, m, s)
      implicit none
      integer, intent(in) :: l, m
      real(kind_physics), intent(in) :: s(3)

      complex(kind_physics), parameter :: i = (zero, one)

      if ((l .lt. 0) .or. (m .lt. -l) .or. (m .gt. l)) then
         OMultipole = 0 ! these are zero per definition
      else
         if (s(1) .gt. 0 .or. l .gt. 0) then
            OMultipole = s(1)**real(l, kind=kind_physics) * &
                         Ptilda(l, m, s(2)) * exp(-i * real(m, kind=kind_physics) * s(3))
         else
   ...
   !>
   complex(kind_physics) function MTaylor(l, m, s)
      implicit none
      integer, intent(in) :: l, m
      real(kind_physics), intent(in) :: s(3)
   ...
   !>
   complex(kind_physics) function omega(l, m, s, q)
      implicit none
      integer, intent(in) :: l, m
      real(kind_physics), intent(in) :: s(3)
      real(kind_physics), intent(in) :: q

      omega = real(q, kind=kind_physics) * OMultipole(l, m, s)
   end function omega
   ...
   !>
   complex(kind_physics) function mu(l, m, s, q)
      implicit none
      integer, intent(in) :: l, m
      real(kind_physics), intent(in) :: s(3)
      real(kind_physics), intent(in) :: q

      mu = real(q, kind=kind_physics) * MTaylor(l, m, s)
   end function mu
   ...
   subroutine fmm_sum_lattice_force(pos, e_lattice, phi_lattice)
      ...
      real(kind_physics), intent(out) ::  e_lattice(3), phi_lattice

      complex(kind_physics) :: mu_shift(1:fmm_array_length_taylor), O_R(1:fmm_array_length_multipole)
      integer :: l, m
      real(kind_physics) :: R(3)
      real(kind_physics) :: S(3)
      real(kind_physics) :: prefact
      integer :: p, ibox
      real(kind_physics), parameter :: pi = acos(-one)
      real(kind_physics) :: etmp(3), phitmp, delta(3)
   ...
   !> @param[in] A table
   !>
   complex(kind_physics) function tbl(A, l, m, Lmax)
      implicit none
      integer, intent(in) :: l, m, Lmax
      complex(kind_physics), intent(in) :: A(*)
   ...
   function M2M(O_a, O_b)
      implicit none
      complex(kind_physics), intent(in) :: O_a(1:fmm_array_length_multipole)
      complex(kind_physics), intent(in) :: O_b(1:fmm_array_length_multipole)
      complex(kind_physics), dimension(1:fmm_array_length_multipole) :: M2M

      complex(kind_physics) :: t
      integer :: l, m, j, k
   ...
   function M2L(M_b, O_a)
      implicit none
      complex(kind_physics), intent(in) :: M_b(1:fmm_array_length_taylor)
      complex(kind_physics), intent(in) :: O_a(1:fmm_array_length_multipole)
      complex(kind_physics), dimension(1:fmm_array_length_taylor) :: M2L
   ...
   function L2L(O_b, M_r, max_l)
      implicit none
      complex(kind_physics), intent(in) :: O_b(1:fmm_array_length_multipole)
      complex(kind_physics), intent(in) :: M_r(1:fmm_array_length_taylor)
      complex(kind_physics), dimension(1:fmm_array_length_taylor) :: L2L
   ...
   function UL(L)
      implicit none
      complex(kind_physics), intent(in) :: L(1:fmm_array_length_taylor)
      complex(kind_physics), dimension(1:fmm_array_length_taylor) :: UL
      real(kind_physics) :: scalefac

      integer :: ll, mm

      scalefac = real(2 * ws + 1, kind=kind_physics)

      do ll = 0, Lmax_taylor
         do mm = 0, ll
            UL(tblinv(ll, mm, Lmax_taylor)) = L(tblinv(ll, mm, Lmax_taylor)) / scalefac**real(ll + 1, kind=kind_physics)
         end do
      end do
   ...
   !>
   complex(kind_physics) function MstarFunc(l, m)
      implicit none
      integer, intent(in) :: l, m
      real(kind_physics) :: rpart(num_neighbour_boxes), ipart(num_neighbour_boxes), rp, ip, s(3)
      complex(kind_physics) :: tmp
      complex(kind_physics), parameter :: ic = (zero, one)
      ...
      do i = 1, num_neighbour_boxes
         ...
         rpart(i) = real(tmp, kind=kind_physics)
         ipart(i) = real(aimag(tmp), kind=kind_physics)
      end do
   ...
   !> with some overhead to avoid numerical elimination of small values
   !>
   complex(kind_physics) function LstarFunc(l, m)
      implicit none
      integer, intent(in) :: l, m
      real(kind_physics) :: rpart(num_neighbour_boxes * (num_neighbour_boxes - 1)), ipart(num_neighbour_boxes * (num_neighbour_boxes - 1)), rp, ip, s(3)
      complex(kind_physics) :: tmp
      complex(kind_physics), parameter :: ic = (zero, one)
      integer :: i, ii, k
      ...
      do i = 1, num_neighbour_boxes - 1 ! central box is being omitted in this loop
         do ii = 1, num_neighbour_boxes
            ...
            ! we store the summands and order them before performing the sum
            ! to avoid numeric elimination
            rpart(k) = real(tmp, kind=kind_physics)
            ipart(k) = real(aimag(tmp), kind=kind_physics)
         end do
      end do
   ...
   function cartesian_to_spherical(cartesian)
      implicit none
      real(kind_physics), intent(in)  :: cartesian(3)
      real(kind_physics), dimension(3) :: cartesian_to_spherical, c

      c = real(cartesian, kind=kind_physics)

      cartesian_to_spherical(1) = sqrt(dot_product(c, c))

      if (cartesian_to_spherical(1) .eq. 0) then
         cartesian_to_spherical(2) = one
      else
         cartesian_to_spherical(2) = c(3) / cartesian_to_spherical(1)
      end if

      if ((c(1) .eq. 0) .and. (c(2) .eq. 0)) then
   ...
   subroutine PrintTable(s, T, Lmax)
      implicit none
      complex(kind_physics), intent(in) :: T(:)
   ...
   !>
   real(kind_physics) function LegendreP(l, m, x)
      use module_debug
      implicit none
      integer, intent(in) :: l, m
      real(kind_physics) ::x
      ...
      integer :: i, ll
      real(kind_physics) :: fact, pll, pmm, pmmp1, somx2
   ...
   subroutine sort_abs(arr)
      implicit none
      real(kind_physics), intent(inout) :: arr(:)
      integer :: i, n
   ...
   subroutine chop(a)
      implicit none
      complex(kind_physics), intent(inout) :: a(:)
      integer :: i
      complex(kind_physics), parameter :: ic = (zero, one)
      real(kind_physics) :: re, im
      ...
      do i = 1, size(a)
         re = real(a(i), kind=kind_physics)
         im = real(aimag(a(i)), kind=kind_physics)
