...
contains
   ...
   function mac(particle, node, dist2, boxlength2)
      ...
      case (3)
         ...
         mac = dist2 .gt. particle%results%maxdist2 + sqrt(12.*particle%results%maxdist2 * boxlength2) + 3.*boxlength2
      ...
      case (4)
         ...
         !
         mac = ((dist2 .gt. particle%results%maxdist2 + sqrt(12.*particle%results%maxdist2 * boxlength2) + 3.*boxlength2) .or. &
                (dist2 .gt. 4.*node%h**2 + sqrt(24.*node%h * boxlength2) + 3.*boxlength2))
   ...
   subroutine update_nn_list(particle, node, node_idx, d, dist2)
      ...
      case (3)
         ...
         if (dist2 .lt. particle%results%maxdist2) then
            ! add node to NN_list
      ...
      case (4)
         ...
         if ((dist2 .lt. particle%results%maxdist2) .or. (dist2 .lt. 4.*node%h * node%h)) then
            ! add node to NN_list
            particle%results%neighbour_nodes(particle%results%maxidx) = node_idx
            particle%results%dist2(particle%results%maxidx) = dist2
            particle%results%dist_vector(:, particle%results%maxidx) = d
            particle%results%maxidx = particle%results%maxidx + 1
            if (particle%results%maxidx .gt. max_neighbour_particles) then
