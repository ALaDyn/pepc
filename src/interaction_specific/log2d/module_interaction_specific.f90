...
module module_interaction_specific
  ...
  subroutine multipole_from_particle(x, p, c, m)
    ...
    real(kind_physics), intent(out) :: c(3)
    type(t_multipole_moments), intent(out) :: m
    ...
    if (p%source_kind == CALC_FORCE_SOURCE_KIND_PARTICULAR) then
      c = x
      m= t_multipole_moments(p%q, abs(p%q), omega0, 0.)
    else
      za = p%ra(1) + ic * p%ra(2)
      zb = p%rb(1) + ic * p%rb(2)
      zc = x(1) + ic * x(2)
      ...
      c = x
      m = t_multipole_moments(p%q * l, abs(p%q) * l, omega, 0.)
    end if
#else
    c = x
    m = t_multipole_moments(p%q, abs(p%q), omega0, 0.)
  ...
  subroutine calc_force_log_2D(t, d, ex, ey, phi)
    ...
    type(t_multipole_moments), intent(in) :: t
