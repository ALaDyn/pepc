...
module module_interaction_specific
     ...
     private
      ...
      !> (different) force calculation routines
      !>
      subroutine calc_force_per_interaction_with_leaf_s(particle, node, node_idx, delta, dist2, vbox)
        use module_pepc_types
        use treevars
        use module_coulomb_kernels
        implicit none
      ...
      subroutine calc_force_per_interaction_with_leaf_v(np, particles, node, node_idx, deltas, dist2s, vbox)
        ...
        !!            !  and Kelbg for particle-particle interaction
        !!      ! It's a leaf, do direct summation with kelbg
        !!      call calc_force_kelbg_3D_direct(np, particles(:), node, deltas(:,:), dist2s(:), kelbg_invsqrttemp, exyz(:,:), phic(:))
          case default
            exyz = 0.
            phic = 0.
        end select
