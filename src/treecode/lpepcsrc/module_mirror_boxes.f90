...
contains
   ...
   subroutine constrain_periodic(particles)
      ...
      if (simplelattice) then
         ...
         do p = 1, np_local
            ...
            if (any(lattice_coord .gt. 1.D+0) .or. any(lattice_coord .lt. 0.D+0)) then
   ...
   logical function system_uses_principal_axes()
      ...
      system_uses_principal_axes = (t_lattice_1(1) .gt. 0.) .and. &
                                   (t_lattice_1(2) .eq. 0.) .and. &
                                   (t_lattice_1(3) .eq. 0.) .and. &
                                   (t_lattice_2(1) .eq. 0.) .and. &
                                   (t_lattice_2(2) .gt. 0.) .and. &
                                   (t_lattice_2(3) .eq. 0.) .and. &
                                   (t_lattice_3(1) .eq. 0.) .and. &
                                   (t_lattice_3(2) .eq. 0.) .and. &
                                   (t_lattice_3(3) .gt. 0.)
   end function
   ...
   logical function system_is_unit_cube()
      ...
      system_is_unit_cube = system_uses_principal_axes() .and. &
                            (normsq(t_lattice_1) .eq. 1.0) .and. &
                            (normsq(t_lattice_2) .eq. 1.0) .and. &
                            (normsq(t_lattice_3) .eq. 1.0)
   end function
   ...
   logical function check_lattice_boundaries(particles)
      ...
      do p = 1, size(particles)
         ...
         if (any(((lattice_coord .gt. 1.D+0) .or. (lattice_coord .lt. 0.D+0)) .and. periodicity)) then
