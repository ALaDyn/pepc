...
!
! Copyright (C) 2002-2020 Juelich Supercomputing Centre,
...
contains
   ...
   DEBUG_ELEMENTAL function parent_key_from_key(key)
      ...
      DEBUG_ASSERT(level_from_key(key) .gt. 0)
   ...
   DEBUG_PURE function shift_key_by_level(key, lvl)
      ...
      DEBUG_ASSERT(0 .le. level_from_key(key) + lvl)
      DEBUG_ASSERT(level_from_key(key) + lvl .le. maxlevel)
   ...
   DEBUG_PURE function is_ancestor_of_with_level(ka, la, kc, lc)
      ...
      DEBUG_ASSERT(level_from_key(ka) .eq. la)
      DEBUG_ASSERT(level_from_key(kc) .eq. lc)

      is_ancestor_of_with_level = lc .ge. la
      if (.not. is_ancestor_of_with_level) return
      is_ancestor_of_with_level = ka .eq. shift_key_by_level(kc, la - lc)
   end function
   ...
   DEBUG_PURE function is_ancestor_of_particle(ka, kp)
      ...
      DEBUG_ASSERT(level_from_key(kp) .eq. maxlevel)
   ...
   DEBUG_PURE function is_ancestor_of_particle_with_level(ka, la, kp)
      ...
      DEBUG_ASSERT(level_from_key(ka) .eq. la)         ! key of ancester (ka) shoud be on level la
      DEBUG_ASSERT(level_from_key(kp) .eq. maxlevel)   ! particles keys (kp) always have level maxlevel
   ...
   subroutine key_to_intcoord_hilbert(key, ic)
      ...
      DEBUG_ASSERT(idim .eq. 2 .or. idim .eq. 3)
