...
!     enddo
      ...
      if ((y_wn - (yt + (boxl / 2))) .lt. 0) then
         e_dy = yt + boxl - y_wn
      else
         e_dy = yt - y_wn
      end if

      if ((z_wn - (zt + (boxl / 2))) .lt. 0) then
         e_dz = zt + boxl - z_wn
      else
         e_dz = zt - z_wn
      end if
...
!     write(*,*) "MAC3"
      ...
      mac_ok = (dist**5 * field_old * alpha .gt. walk_abs_charge * boxl**3)
