    ...
    contains
      ...
      recursive subroutine eager_collect_traverse(node)
        ...
        do
          ...
          ! check MAC
          if (.not. (mac(IF_MAC_NEEDS_PARTICLE(request%particle) n_ptr%multipole_moments, dist2, t%boxlength2(n_ptr%level)) .or. tree_node_is_leaf(n_ptr))) then
            ! resolve the node
    ...
    recursive subroutine unpack_children(parent_idx, toplvl)
      ...
      do
        if (child_data(ic)%level < lvl) then
          ...
          call tree_node_set_has_remote_contributions(t%nodes(newnode), .true.)
