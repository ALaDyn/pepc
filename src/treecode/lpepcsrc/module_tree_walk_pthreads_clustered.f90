...
contains
   ...
   subroutine tree_walk_run(vbox_)
      ...
      DEBUG_ASSERT(size(threaddata) .eq. num_walk_threads)
   ...
   subroutine identify_particle_clusters()
      ...
      do k = 1, 2
         ...
         do while (i .le. np)
            ...
            if (k .eq. 2) particle_clusters(num_clusters)%first = i
            if (i .lt. np) then
               cut_key = bpi(particle_data(i)%key, particle_data(min(i + max_particles_per_cluster, np))%key)
               do while ((iand(particle_data(i)%key, cut_key) .ne. cut_key) .and. i .lt. np)
                  i = i + 1
               end do
            else
               i = i + 1
            end if
            if (k .eq. 2) particle_clusters(num_clusters)%last = i - 1
         end do
         ! allocate enough space for storing them
         if (k .eq. 1) allocate (particle_clusters(num_clusters))
      end do
   end subroutine
   ...
   function walk_worker_thread(arg) bind(c)
      ...
      shared_core = (my_processor_id .eq. walk_tree%communicator%processor_id) .or. &
      ...
      if (my_clusters_per_thread .gt. 0) then
         ...
         do while (clusters_active .or. clusters_available)
            ...
            do i = 1, my_clusters_per_thread
               ...
               if (contains_cluster(i)) then
                  ...
                  if (defer_list_new_tail .gt. total_defer_list_length) then
   ...
   contains
      ...
      function get_first_unassigned_cluster()
         ...
         if (next_unassigned_cluster_local .lt. num_clusters + 1) then
   ...
   function walk_single_cluster(cluster, defer_list_old, defer_list_entries_old, &
      ...
      walk_single_cluster = (todo_list_entries .eq. 0) .and. (defer_list_entries_new .eq. 0)
   ...
   contains
...
#ifndef NO_SPATIAL_INTERACTION_CUTOFF
            if (any(abs(pdelta) .ge. spatial_interaction_cutoff)) cycle
#endif

            if (pdist2 .gt. 0.0_8) then ! not self, interact
      ...
      function todo_list_pop(node)
         ...
         todo_list_pop = (todo_list_entries .gt. 0)
      ...
      function todo_list_push_siblings(node) result(res)
         ...
         res = (todo_list_entries + 8 .le. todo_list_length)
         if (res) then
            n = node
            do
      ...
      subroutine defer_list_parse_and_compact()
         ...
         iold = 1
         do
            if (iold .gt. defer_list_entries_old) return
