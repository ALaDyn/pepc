...
contains
   ...
   subroutine domain_decompose(d, b, n, particles, bp, c)
      ...
      if (d%npnew .lt. 2) then
         DEBUG_ERROR('("rank has less than two particles after sorting (had ", I8, " before). Did you initialise particle field %work correctly? --.gt.aborting")', d%npold)
      end if

      if (d%npnew .gt. d%nppmax) then
         DEBUG_ERROR('("More than nppm particles after sorting: nppm = ", I0, ".lt.npp = ",I0,". All local particle fields are too short. Aborting.")', d%nppmax, d%npnew)
      end if
      ...
      ! perform permute
      do i = 1, d%npold
         DEBUG_ASSERT(level_from_key(ship_parts(i)%key) .eq. maxlevel)
      end do
      ...
      do i = 1, d%npnew
         DEBUG_ASSERT(level_from_key(get_parts(i)%key) .eq. maxlevel)
      end do
      ...
      if ((local_keys(d%npnew) - local_keys(1) + 1) .lt. d%npnew) then
      ...
      do i = 1, d%npnew
         if (key_diffs(i) .lt. 1) then
            ...
            do j = 1, max(d%npnew - i, i)
               if (key_diffs(max(1_kind_particle, i - j)) .gt. 1) then
               ...
               else if (key_diffs(min(d%npnew - 1, i + j)) .gt. 1) then
