...
!         if(t%comm_env%rank == 219) write(779,*) 'tree_exchange ', get_mult(i)%key, get_mult(i)%leaves, get_mult(i)%owner
      ...
      DEBUG_ASSERT_MSG(j .eq. nbranch, *, j, num_local_branch_nodes)
   ...
   subroutine tree_exchange_branches(t, p, bp, bn)
      ...
      do i = 1, t%nbranch
         ...
         if (branch%owner .ne. t%comm_env%rank) then
...
!         end if
         ...
         do while (i .le. nsub)
            ...
            groupstart = i
            do
               if (.not. i + 1 .le. nsub) exit
               if (.not. parent_key_from_key(sub_key(i + 1)) .eq. current_parent_key) exit
               i = i + 1
            end do
            groupend = i
...
!               if (t%nodes(sorted_sub_nodes(k))%owner .eq. 219) write(debug_ipefile,'(a,o22)') 'grouped ', t%nodes(sorted_sub_nodes(k))%key
   ...
   subroutine tree_build_from_particles(t, p, bp)
      ...
      if (any(p(:)%node_leaf .eq. NODE_INVALID)) then
   ...
   contains
      ...
      recursive subroutine insert_helper(t, k, l, ki, cur, pi)
         ...
         if (cur .le. ubound(ki, dim=1, kind=kind_particle)) then
            first_is_child = is_ancestor_of_particle(k, l, ki(cur)%key)
         else
            first_is_child = .false.
         end if

         if (cur + 1 .le. ubound(ki, dim=1, kind=kind_particle)) then
         ...
         if (.not. (first_is_child .or. second_is_child)) then ! no particles below this key
            ...
            if (ki(cur)%idx .ne. 0) then ! boundary particles have idx == 0, do not really need to be inserted
