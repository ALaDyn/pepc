...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine validate_n_nearest_neighbour_list(np_local, particles, &
    ...
    real*8 :: dist2
    integer*8 :: node_id, neighbour_id
    ...
    allocate( distances2( num_neighbour_particles +1, np_local), &
                          positions( 3, num_neighbour_particles +1, np_local), &
                          particle_ids(num_neighbour_particles +1, np_local) )
    ...
    do actual_pe =0 , n_cpu-1                                  ! each process sends data
       ...
       if( actual_pe .eq. my_rank ) then
          ...
          id_buffer(1:np_local)          = particles( 1:np_local )%data%particle_id

       end if

       call MPI_BCAST( coord_buffer,   all_np_local( actual_pe+1 )*3, MPI_REAL8,       actual_pe, MPI_COMM_WORLD, ierr )
       call MPI_BCAST(    id_buffer,   all_np_local( actual_pe+1 )  , MPI_INTEGER8,    actual_pe, MPI_COMM_WORLD, ierr )
...
!                     + ( z_buffer( remote_particle_index )  - particles( local_particle_index )%x(3) ) **2
                ...
                if( dist2 < distances2(maxdist, local_particle_index ) ) then
                   ...
                   particle_ids(maxdist, local_particle_index )   = id_buffer( remote_particle_index )
                   tmp_loc = maxloc(distances2(1:num_neighbour_particles+1, local_particle_index ) )
                   maxdist = tmp_loc(1)        !< this is needed because maxloc returns an array
                end if
    ...
    deallocate( coord_buffer, id_buffer )
    ...
    do local_particle_index = 1, np_local
       ...
       tmp_int8 = particle_ids( tmp_loc(1), local_particle_index )
       particle_ids( tmp_loc(1), local_particle_index ) = particle_ids( num_neighbour_particles+1, local_particle_index )
       particle_ids( num_neighbour_particles+1, local_particle_index ) = tmp_int8
    ...
    do local_particle_index = 1, np_local
       do index_in_result_neighbour_list = 1, num_neighbour_particles
          ...
          do index_in_test_neighbour_list = 1, num_neighbour_particles   ! ignore particle self (num_neighbour_particles+1)
             ...
             neighbour_id = particle_ids( index_in_test_neighbour_list, local_particle_index )

             if( node_id .eq. neighbour_id ) then
                found = .true.
                exit
             end if
          end do
          ...
          if( .not. found ) then
             ...
             write( 76 , '(a,i6,a,i19.19,a,i6)' ) "for particle ", particles(local_particle_index)%label, " the neighbour with id ", &
                  node_id, " was not found in the result neighbour list"
...
!             WRITE( 76, *) '  ', stored_keys( i , 1:n_nn)
    ...
    deallocate( distances2, positions, particle_ids )
