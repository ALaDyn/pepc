...
module module_particles_resize
   use module_helper
...
contains
   ...
   subroutine sort_sibling_species(particles, sibling_cnt, parent_no, unique_parents)
      ...
      !begin sorting
      sort_width = 1
      do while (sort_width .lt. buffer_size)

         iLeft = 1
         do while (iLeft .le. buffer_size)
            ...
            do i = iLeft, iEnd !!!CHECK THE EXTENT, Also consider if n < largest sort_width
               if (j .lt. iRight) then
                  if (k .gt. iEnd) then
                     ...
                     j = j + 1
                  else if (copied_local_buffer(j)%data%species .le. copied_local_buffer(k)%data%species) then
                     sorting_buffer(i) = copied_local_buffer(j)
                     j = j + 1
                  else
            ...
            do i = iLeft, iEnd !!!CHECK THE EXTENT, Also consider if n < largest sort_width
               if (j .lt. iRight) then
                  if (k .gt. iEnd) then
                     ...
                     j = j + 1
                  else if (particles(j)%key .le. particles(k)%key) then
                     sorting_buffer(i) = particles(j)
                     j = j + 1
                  else
            ...
            do i = iLeft, iEnd !!!CHECK THE EXTENT, Also consider if n < largest sort_width
               if (j .lt. iRight) then
                  if (k .gt. iEnd) then
                     sorting_buffer(i) = particles(j)
                     j = j + 1
                  else if (abs(particles(j)%data%q) .le. abs(particles(k)%data%q)) then
                     sorting_buffer(i) = particles(j)
                     j = j + 1
                  else
                     sorting_buffer(i) = particles(k)
   ...
   subroutine defined_siblings_number_grouping(particles, sibling_upper_limit, sibling_cnt, unique_parents)
      ...
      if (sibling_upper_limit .lt. max_siblings) then
         do while (sibling_upper_limit .lt. max_siblings)
            sibling_cnt = 0
            ...
            max_siblings = 0
            do i = 1, unique_parents
               if (sibling_cnt(i) .gt. max_siblings) then
                  max_siblings = sibling_cnt(i)
               end if
            end do
         end do
      else if (sibling_upper_limit .gt. max_siblings) then
         unique_parents = 1
         level = -1
   ...
   subroutine merge_replace_particles_list(particles, buffer, merged_cnt)
      ...
      if (new_size .lt. ll_buffer_size) then
         tail = new_size
      else
         tail = ll_buffer_size
      end if
