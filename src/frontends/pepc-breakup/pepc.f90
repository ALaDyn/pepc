...
!====================== save the resolved 'e' from tree traverse================
         ...
         if (particles(i)%data%species .eq. 0) then
...
!       initially carried by super-particle!
               ...
               j = 0
               do while (j .lt. collision_checks)
...
!$OMP END PARALLEL
      ...
      if (root) then
         if ((new_particle_cnt .gt. 0) .or. (swapped_num .ne. 0 .or. electron_num .ne. 0)) then
         ...
         if ((new_particle_cnt .gt. 0) .or. (swapped_num .ne. 0)) then
...
!      if (root) print *, "Total particles: ", tnp
      ...
      do i = 1, size(particles)
         ...
         do j = 1, size(local_min_x)
            if (particles(i)%x(j) .lt. local_min_x(j)) then
               local_min_x(j) = particles(i)%x(j)
            end if

            if (particles(i)%x(j) .gt. local_max_x(j)) then
      ...
      if (tnp .gt. merging_threshold_tnp .and. MOD(step + itime_in + 1, relaxation_steps) .eq. 0) then!steps_since_last > 250000) then
