...
contains
   ...
   subroutine test_ionization(particle, guide, new_particle, electron_count)
      ...
      if (particle%data%age .gt. 6000) then
         ...
         if (ll_elem_gen .eq. 0 .and. new_particle .gt. 0) then
   ...
   subroutine boris_velocity_update(particle, dt)
      ...
      if (B_mag .gt. 0.0_kind_physics) then
   ...
   subroutine determine_cross_sections(particle, sigma_vec, guide_CS)
      ...
      do while (associated(temp_guide))
         ...
         if (energy .lt. temp_guide%CS(i, 1) .or. energy .gt. temp_guide%CS(table_rows, 1)) then
            ...
            do while (energy .gt. temp_guide%CS(i, 1))
   ...
   subroutine determine_cross_sections_ext(particle, sigma_vec, guide_CS, slopes_table)
      ...
      do while (associated(temp_guide))
         ...
         if (energy .lt. temp_guide%CS(i, 1)) then
            if (CS_table_no .ne. 1) then
               sigma_vec(CS_table_no) = sigma_vec(CS_table_no - 1)
            else
               sigma_vec(CS_table_no) = 0.0_kind_physics
            end if
         else if (energy .gt. temp_guide%CS(table_rows, 1)) then
   ...
   subroutine momentum_partition_merging(particles, sibling_cnt, sibling_upper_limit, &
      ...
      do i = istart, istop
         ...
         if (particles(i)%data%v(1) .gt. 0.0_kind_physics) then
            if (particles(i)%data%v(2) .gt. 0.0_kind_physics) then
               if (particles(i)%data%v(3) .gt. 0.0_kind_physics) then
                  direction = 1
               else
                  direction = 2
               end if
   ...
   subroutine momentum_partition_merging_alt(particles, sibling_cnt, sibling_upper_limit, &
      ...
      do i = istart, istop
         ...
         ! NOTE: momentum direction filtering
         if (V_temp(1) .gt. 0.0_kind_physics) then
            if (V_temp(2) .gt. 0.0_kind_physics) then
               if (V_temp(3) .gt. 0.0_kind_physics) then
                  direction = 1
               else
                  direction = 2
               end if
   ...
   subroutine momentum_partition_merging_fine(particles, sibling_cnt, sibling_upper_limit, &
      ...
      do i = istart, istop
         ...
         if (theta_idx .lt. 1) then
            theta_idx = 1
         else if (theta_idx .gt. theta_div) then
            theta_idx = theta_div
         end if

         if (phi_idx .lt. 1) then
            phi_idx = 1
         else if (phi_idx .gt. phi_div) then
            phi_idx = phi_div
         end if
   ...
   subroutine age_elastic_merging(direction, directional_buffer, direction_cnt, merged_guide, merged_cnt, species, parent_key)
      ...
      if (m_i .ne. 0) then
         ...
         if (ll_elem_gen .eq. 0 .and. merged_cnt .gt. 0) then
         ...
         if (ll_elem_gen .le. size(merged_guide%tmp_particles)) then
...
!      energy_threshold(6) = 60.0
      ...
      if (direction_cnt .gt. 2) then
         ...
         do i = 1, direction_cnt
            ...
            if (kin_e .ge. energy_threshold(filtered_instance)) then
               f_i = filtered_instance
            else
               do while (kin_e .lt. energy_threshold(f_i))
                  f_i = f_i - 1
               end do
            end if
            if (f_i .lt. 1 .or. f_i .gt. filtered_instance) print *, "Checking f_i: ", f_i, kin_e
            grouped_count(f_i) = grouped_count(f_i) + 1
            energy_collector(f_i, grouped_count(f_i)) = directional_buffer(direction, i)
            if (weight .gt. max_weight(f_i)) then
...
!        end if
         ...
         do j = j_start, filtered_instance
            if (grouped_count(j) .ne. 0) then
               if (grouped_count(j) .gt. 2) then
                  if (max_weight(j) .gt. 1.0_kind_physics) then
                  ...
                  if (merge_ratio .ge. 0.5 .or. remainder .le. 2) then
                     do i = IStart, IStop
                        m_i = m_i + 1
                        merged_buffer(m_i) = energy_collector(j, i)
                        merged_buffer(m_i)%label = 0
                     end do
                  else if (merge_ratio .lt. 0.5 .and. remainder .gt. 2) then
            ...
            if (grouped_count(j) .ne. 0) then
               if (grouped_count(j) .gt. 2) then
                  ...
                  if (max_weight(j) .gt. 1.0_kind_physics) then
                     ...
                     if (weight_counts(min_weight) .lt. merge_collector_size) then
                        do i = 1, grouped_count(j)
                           m_i = m_i + 1
                           merged_buffer(m_i) = energy_collector(j, i)
                           merged_buffer(m_i)%label = 0
                        ...
                        if (remainder .gt. CEILING(grouped_count(j) * merge_ratio)) then
            ...
            if (grouped_count(j) .ne. 0) then
               if (grouped_count(j) .gt. 4) then
                  ...
                  if (max_weight(j) .gt. 1.0_kind_physics) then
                     ...
                     if (weight_counts(weight_indexing) .le. 5) then
                     ...
                     if (merge_ratio .gt. 0.5_kind_physics) then
...
!      deallocate(energy_threshold)
   ...
   subroutine resolve_elastic_merge_momentum(input_buffer, input_cnt, merged_buffer, m_i, group_index)
      ...
      do i = 1, input_cnt
         ...
         do j = 1, 3
            if (abs(input_buffer(i)%data%v(j)) .gt. max_vec(j)) then
      ...
      if (vel_mag2 .ne. 0.0_kind_physics) then
         ...
         if (w1 .lt. (total_weight - vel_mag2 / sum_particle%data%f_e(1))) then
         ...
         if (Chi_arg .gt. 1.0_kind_physics .and. Chi_arg .lt. (1.0_kind_physics + 1e-15)) then
   ...
   subroutine resolve_elastic_merge_momentum_nonphys(input_buffer, input_cnt, merged_buffer, m_i, group_index)
      ...
      if (abs((w1 * vel_mag**2.+w2 * vel_mag**2.) - sum_particle%data%f_e(1)) .gt. 1.0_kind_physics) print *, "energy issue"
   ...
   subroutine resolve_incident_electron_outcome(particle, init_vel, ran_num, collision_type, new_weight)
      ...
      case (1) ! elastic scattering (no additional electron, no byproducts)
         particle%data%v = scatter_angle_method(vel_mag, unit_inc_vel, K_E, ran_num)
      ...
      case (2) ! rotational excitation of H2 molecule, electron will lose the transition energy
         ...
         particle%data%v = scatter_angle_method(reduced_vel_mag, unit_inc_vel, K_E, ran_num)
      ...
      case (4) ! nondissociative ionization (1 additional electron, 1 byproduct)
         if (scatter_mode .eq. 1) then
            scatter_loss = 2.*(1.-cos_Chi) / H2_mass
         end if
         reduced_vel_mag = sqrt(vel_mag**2 - 2.*(IE_H2_ion + scatter_loss) / e_mass)
         !  print *, "incident momentum: ", scaled_mass* reduced_incident

         temp_vel = scatter_angle_ionise_method(reduced_vel_mag, unit_inc_vel, K_E, ran_num)
         reduced_incident = reduced_vel_mag * unit_inc_vel
      ...
      case (6) ! H2 molecule dissociation into H(1s) atoms (no additional electron, no byproducts)
         ! update velocity to indicate scattering into random angle
         reduced_vel_mag = sqrt(vel_mag**2 - 2.*Disso_H1 / e_mass)

         particle%data%v = scatter_angle_neutralDisso_method(reduced_vel_mag, unit_inc_vel, K_E, ran_num)
         particle%data%age = 0.0_8

      case (7) ! H2 molecule dissociation into H(1s) & H(2s) atoms (no additional electron, no byproducts)
         ! update velocity to indicate scattering into random angle
         reduced_vel_mag = sqrt(vel_mag**2 - 2.*Disso_H2 / e_mass)
   ...
   function scatter_angle_method(reduced_vel_mag, unit_inc_vel, K_E, ran_num) result(scattered_v)
      implicit none
      real(kind_physics), intent(in) :: reduced_vel_mag, unit_inc_vel(3), K_E, ran_num(8)
      real(kind_physics) :: scattered_v(3), scatter_loss, rot_axis(3), upd_vel_mag
      real(kind_physics) :: xi, polar_theta, polar_phi, cos_Chi, Chi, temp_vel(3), temp_vel1(3), prefac
      real(kind_physics), parameter :: H2_mass = 3674.43889456 ! H2/e mass ratio

      polar_theta = 2.0 * pi * ran_num(4)
      polar_phi = acos(2.0 * ran_num(3) - 1.)

      ! NOTE: scatter modes are
      !       0: random scatter
      !       1: Vahedi & Surendra 1995
      !       2: Okhrimovvsky 2022
      !       3: Anti_parallel scatter
      select case (scatter_mode)
      case (0)
         scattered_v(1) = reduced_vel_mag * sin(polar_phi) * cos(polar_theta)
         scattered_v(2) = reduced_vel_mag * sin(polar_phi) * sin(polar_theta)
         scattered_v(3) = reduced_vel_mag * cos(polar_phi)
      ...
      case (1)
         ...
         upd_vel_mag = sqrt(reduced_vel_mag**2 - 2.*scatter_loss / e_mass)
         ...
         scattered_v = upd_vel_mag * (cos_Chi * unit_inc_vel + &
      ...
      case (2)
         ...
         scattered_v = reduced_vel_mag * (cos_Chi * unit_inc_vel + &
                                          sin(polar_theta) * prefac * temp_vel + &
                                          cos(polar_theta) * prefac * temp_vel1)

      case (3)
         scattered_v = -1.0 * reduced_vel_mag * unit_inc_vel
      end select
   end function scatter_angle_method

   function scatter_angle_neutralDisso_method(reduced_vel_mag, unit_inc_vel, K_E, ran_num) result(scattered_v)
      implicit none
      real(kind_physics), intent(in) :: reduced_vel_mag, unit_inc_vel(3), K_E, ran_num(8)
      real(kind_physics) :: scattered_v(3), scatter_loss, rot_axis(3), upd_vel_mag
      real(kind_physics) :: xi, polar_theta, polar_phi, cos_Chi, Chi, temp_vel(3), temp_vel1(3), prefac
      ...
      select case (scatter_mode)
      case (0)
         scattered_v(1) = reduced_vel_mag * sin(polar_phi * 0.5) * cos(polar_theta)
         scattered_v(2) = reduced_vel_mag * sin(polar_phi * 0.5) * sin(polar_theta)
         scattered_v(3) = reduced_vel_mag * cos(polar_phi * 0.5)
   ...
   end function scatter_angle_neutralDisso_method

   function scatter_angle_ionise_method(reduced_vel_mag, unit_inc_vel, K_E, ran_num) result(temp_vel)
      implicit none
      real(kind_physics), intent(in) :: reduced_vel_mag, unit_inc_vel(3), K_E, ran_num(8)
      real(kind_physics) :: scattered_v(3), scatter_loss, rot_axis(3), upd_vel_mag, theta, phi
      real(kind_physics) :: xi, polar_theta, polar_phi, cos_Chi, Chi, temp_vel(3), temp_vel1(3), prefac
      real(kind_physics) :: reduced_incident(3)

      polar_theta = 2.0 * pi * ran_num(4)
      polar_phi = acos(2.0 * ran_num(3) - 1.)

      temp_vel = 0.0_kind_physics
      temp_vel1 = 0.0_kind_physics

      select case (scatter_mode)
      case (0)
         reduced_incident = reduced_vel_mag * unit_inc_vel
         call angles_calc(reduced_incident, reduced_vel_mag, theta, phi)
         print *, unit_inc_vel, reduced_vel_mag
   ...
   end function scatter_angle_ionise_method
   ...
   subroutine collision_update(particle, guide, new_particle, electron_count, CS_numbers, r123_ctr, r123_key, charge_count, c_type)
      ...
      if (ran_num(1) .lt. (1 - exp(-1 * nu_prime * dt))) then ! type of collision determined if satisfied
         ...
         i = 1
         do while (ran_num(2) .gt. (CS_vector(i) / nu_prime))
            i = i + 1
            if (i .gt. size(CS_vector)) then
               i = 0
               EXIT
            end if
         end do
   ...
   recursive subroutine filter_and_swap(particles, geometry, current_index, head_i, tail_i, swapped_cnt, charge_count, break_check)
      ...
      case (1) ! box boundary
         ...
         if (current_index .lt. target_swap) then
            if ((abs(x) .gt. box_dim * 0.5) .or. (abs(y) .gt. box_dim * 0.5)) then
               particles(current_index) = particles(target_swap)
               swapped_cnt = swapped_cnt + 1
            else
               if (particles(current_index)%x(3) .gt. 0.0) then
                  charge_count(1) = charge_count(1) + particles(current_index)%data%q
                  particles(current_index) = particles(target_swap)
                  swapped_cnt = swapped_cnt + 1
               else if (particles(current_index)%x(3) .lt. -box_l) then
            ...
            if (init_swap_cnt .ne. swapped_cnt) then
      ...
      case (2) ! cylinder boundary
         ...
         if (current_index .lt. target_swap) then
            if (radius .lt. cyl_radius) then
               if (particles(current_index)%x(3) .lt. -cyl_length) then
                  if (radius .lt. plate_radius) then
                     charge_count(2) = charge_count(2) + particles(current_index)%data%q
                     particles(current_index) = particles(target_swap)
                     swapped_cnt = swapped_cnt + 1
                  else
               ...
               else if (particles(current_index)%x(3) .gt. 0.0) then
                  if (radius .lt. plate_radius) then
                     charge_count(1) = charge_count(1) + particles(current_index)%data%q
                     if (particles(current_index)%data%q .lt. 0.0) then
      ...
      case (3) ! torus shape
         ...
         if (current_index .lt. target_swap) then
            if (m_radius .gt. (minor_radius + buffer_zone)) then
               if (particles(current_index)%data%q .lt. 0.0_8) then
                  charge_count(1) = charge_count(1) + particles(current_index)%data%q
               else
                  charge_count(2) = charge_count(2) + particles(current_index)%data%q
               end if
