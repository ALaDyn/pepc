...
contains
   ...
   subroutine angles_calc(vec_in, vel_mag, theta, phi)
      ...
      if ((vec_in(1) .ne. 0.0) .and. (vec_in(2) .ne. 0.0)) then
         if ((vec_in(2) .gt. 0.0) .and. (vec_in(1) .lt. 0.0)) then
            phi = pi + atan(vec_in(2) / vec_in(1))
         else if ((vec_in(2) .lt. 0.0) .and. (vec_in(1) .gt. 0.0)) then
            phi = 2.*pi + atan(vec_in(2) / vec_in(1))
         else if ((vec_in(2) .gt. 0.0) .and. (vec_in(1) .gt. 0.0)) then
            phi = atan(vec_in(2) / vec_in(1))
         else
            phi = pi + atan(vec_in(2) / vec_in(1))
         end if
      else if (vec_in(1) .eq. 0.0) then
         if (vec_in(2) .gt. 0.0) then
            phi = 0.5 * pi
         else
            phi = 1.5 * pi
         end if
      else if (vec_in(2) .eq. 0.0) then
         if (vec_in(1) .gt. 0.0) then
            phi = 0.0
         else
            phi = pi
         end if
   ...
   integer function vtk_step_of_step(step) result(vtk_step)
      ...
      else if (nt - 1 - step .lt. diag_interval) then
   ...
   subroutine preempt_checkpointing(current_wall_time, previous_step_duration, doDiag, particles, step, break)
      ...
      if (my_rank .eq. 0) print *, 'Time left till wall time (seconds): ', difference_to_wall_time

      if (difference_to_wall_time .lt. time_check) then
         if (my_rank .eq. 0) then
            print *, "Pre-empted Checkpointing triggered!"
         end if
         if (.not. doDiag) then
            call write_particles(particles)
            if (my_rank .eq. 0) call write_updated_resume_variables(step + itime_in + 1)
