...
subroutine special_start(iconf)
   ...
   case (1)
      ...
      if (my_rank .eq. 0) write (*, *) "Using special start... case 1 (homogeneous distribution)"
   ...
   case (2)
      ...
      do mpi_cnt = 0, n_cpu - 1
         do p = 1, (fances(mpi_cnt) - fances(mpi_cnt - 1))
            ...
            do while ((xt * xt + yt * yt + zt * zt) .gt. 1.0_8)
               call par_rand(par_rand_res)
   ...
   case (7)
      ...
      if (my_rank .eq. 0) write (*, *) "Using special start... case 7 (3D Madelung Setup) with", n, "particles per edge"
      ...
      do i = 0, n(1) - 1
         do j = 0, n(2) - 1
            do k = 0, n(3) - 1
               ...
               if (mod((globalidx - 1) / 2, n_cpu) .eq. my_rank) then ! distribute pairs of electron and ion, since np_local is constructed a bit weird
                  myidx = myidx + 1
   ...
   case (8)
      ...
      if (my_rank .eq. 0) write (*, *) "Using special start... case 8 (fast homogeneous distribution)"
   ...
   case (9)
      ...
      if (my_rank .eq. 0) write (*, *) "Using special start... case 9 (fast homogeneous distribution with initial velocities set to 0)"
   ...
   case (10)
      ...
      if (my_rank .eq. 0) write (*, *) "Using special start... case 10 (2D- r^-2 distribution)"
   ...
   case (11)
      ...
      if (my_rank .eq. 0) write (*, *) "Using special start... case 11 (1D shock) (Springel 1)"
      ...
      do p = 1, all_part
         ...
         do while (area2 .lt. area1 / real(all_part, 8) * real(p - 1, 8) + offset)
         ...
         if ((p .gt. part_before_me) .and. (p .le. part_including_mine)) then
      ...
      if (my_rank .eq. 0) write (*, *) "Using timestep:", dt
