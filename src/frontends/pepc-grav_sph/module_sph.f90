       ...
       do actual_neighbour = 1, num_neighbour_particles
         actual_node => t%nodes(particles(local_particle_index)%results%neighbour_nodes(actual_neighbour))
          ...
          particles(local_particle_index)%results%rho = particles(local_particle_index)%results%rho + &
               actual_node%multipole_moments%charge *kernel
  ...
  subroutine sph_sum_force(np_local, particles, itime, num_neighbour_boxes, neighbour_boxes)
    ...
    do local_particle_index = 1, np_local
       ...
       do actual_neighbour = 1, num_neighbour_particles
          ...
          h2 = actual_node%multipole_moments%h
          ...
          ! for all dimension
          do dim = 1, idim
             dv(dim) = actual_node%multipole_moments%v(dim) - particles(local_particle_index)%data%v(dim)
          ...
          sound_speed = ( sqrt( const * particles(local_particle_index)%data%temperature )  &
               + sqrt( const * actual_node%multipole_moments%temperature ) )/ 2. ! mean sound_speed
          ...
          if ( use_artificial_viscosity .and. (vr < 0._8) ) then
             ...
             artificial_viscosity = ( - art_vis_alpha * sound_speed * mu + art_vis_beta * mu * mu ) / &
                  ( ( actual_node%multipole_moments%rho +  particles(local_particle_index)%results%rho )/2. )
          ...
          scalar_force = actual_node%multipole_moments%charge * &
               ( &
               const * actual_node%multipole_moments%temperature / actual_node%multipole_moments%rho + &
          ...
          if( sph_debug ) then
             write(50, *) my_rank, local_particle_index, particles(local_particle_index)%x(1), h1, &
                  ...
                  actual_neighbour, actual_node%center(1), distance, dist(1), grad_kernel_1, actual_node%multipole_moments%charge, &
                  actual_node%multipole_moments%temperature, actual_node%multipole_moments%h, actual_node%multipole_moments%rho, &
...
!    write (*,*) 'rdispls:', my_rank, rdispls
    ...
    do i = 1, total_num_requests_from_others
      ...
      do j = 1, np_local
        if (particles(j)%node_leaf == iactual_node) then
          ...
          particles(j)%results%rho, actual_node%multipole_moments%v, actual_node%multipole_moments%temperature )
    ...
    do i= 1, num_request
       ...
       actual_node%multipole_moments%rho         = received_updates(i)%rho
       actual_node%multipole_moments%temperature = received_updates(i)%temperature
       actual_node%multipole_moments%v           = received_updates(i)%v
       actual_node%multipole_moments%h           = received_updates(i)%smoothing_length

    end do

    do i = 1, np_local
       actual_node => t%nodes(particles(i)%node_leaf)
       actual_node%multipole_moments%rho         = particles(i)%results%rho
       actual_node%multipole_moments%temperature = particles(i)%data%temperature
       actual_node%multipole_moments%h           = particles(i)%results%h
    end do
