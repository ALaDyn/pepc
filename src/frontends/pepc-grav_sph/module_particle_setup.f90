...
contains
   ...
   subroutine particle_setup(iconf)
      ...
      select case (iconf)
      case (-1)
         if (my_rank .eq. 0) write (*, *) "Using special start... case -1 (reading data file)"
         call read_particles()

      case (1)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 1 (3D homogeneous distribution)"
         call init_generic(fences)
         call particle_setup_homogeneous(fences, 3)

      case (2)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 2 (one sphere benchmark)"
         call init_generic(fences)
         call particle_setup_sphere(fences)

      case (3)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 3 (two sphere benchmark)"
...
!       call init_fields_zero()
      ...
      case (342)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 342 (42 spheres benchmark)"
...
!       call init_fields_zero()
      ...
      case (4)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 4: Plummer distribution (core cut)"
...
!       call init_fields_zero()
      ...
      case (5)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 5: 2D disc"
...
!       call init_fields_zero()
      ...
      case (6)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 6 (2D homogeneous distribution)"
         call init_generic(fences)
         call particle_setup_homogeneous(fences, 2)

      case (7)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 7 (3D homogeneous distribution xyz periodic)"
!       call init_generic(fences)
!       call particle_setup_homogeneous(fences, 3)

      case (8)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 8 (fast 3D homogeneous distribution)"
         call init_generic(fences)
         call particle_setup_homogeneous_fast(fences, 3)

      case (12)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 12 (1D wave)"
         call init_generic(fences)
         call particle_setup_1d_wave(fences)

      case (13)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 13 (1st 1D shock test)"
         call init_generic(fences)
         call particle_setup_1d_shock_1(fences)

      case (14)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 14 (2nd 1D shock test)"
         call init_generic(fences)
         call particle_setup_1d_shock_2(fences)

      case (15)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 15 (3rd 1D shock test)"
         call init_generic(fences)
         call particle_setup_1d_shock_3(fences)

      case (16)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 16 (3D sphere collapse (grav only) )"
         call init_generic(fences)
         call particle_setup_3d_sphere_grid(fences)
         do_sph = .false.

      case (17)
         if (my_rank .eq. 0) write (*, *) "Using special start... case 16 (3D sphere collapse (grav + sph) )"
         call init_generic(fences)
         call particle_setup_3d_sphere_grid(fences)
...
!             particles(p)%data%v_minus_half = [ 0._8, 0._8, 0._8 ]
   ...
   subroutine particle_setup_sphere(fences)
      ...
      do mpi_cnt = 0, n_cpu - 1
         do p = 1, (fences(mpi_cnt) - fences(mpi_cnt - 1))
            ...
            do while ((xt * xt + yt * yt + zt * zt) .gt. 0.5_8**2)
               xt = par_rand() - 0.5_8
               yt = par_rand() - 0.5_8
               zt = par_rand() - 0.5_8
            end do
   ...
   subroutine particle_setup_1d_wave(fences)
      ...
      do part_counter = 1, all_part
         ...
         do while (area2 .lt. area1 / real(all_part, 8) * real(part_counter - 1, 8) + offset)
         ...
         if ((part_counter .gt. part_before_me) .and. (part_counter .le. part_including_mine)) then
...
!          initialized_v_minus_half = .true.
   ...
   subroutine particle_setup_1d_shock_1(fences)
      ...
      do part_counter = 1, all_part
         ...
         do while (area2 .lt. area1 / real(all_part, 8) * real(part_counter - 1, 8) + offset)
            if (actual_x .le. 0.5) then
               left_y = setup_rho0
            else
               left_y = setup_rho1
            end if

            if ((actual_x + dx) .le. 0.5) then
               right_y = setup_rho0
            else
               right_y = setup_rho1
            end if
         ...
         if ((part_counter .gt. part_before_me) .and. (part_counter .le. part_including_mine)) then
            ...
            if (part_counter .lt. 2 * num_neighbour_particles .or. (all_part - part_counter) .lt. 2 * num_neighbour_particles) then
   ...
   subroutine particle_setup_1d_shock_2(fences)
      ...
      do actual_particle = 1, np_local
         ...
         if (particles(actual_particle)%x(1) .le. 0.5) then
...
! v_minus_half will be computed from v and the force in pepc.f90
         ...
         if ((actual_particle + part_before_me) .lt. 2 * num_neighbour_particles .or. (all_part - (part_before_me + actual_particle)) .lt. 2 * num_neighbour_particles) then
            particles(actual_particle)%data%type = ibset(particles(actual_particle)%data%type, PARTICLE_TYPE_FIXED)
         end if
...
!    write(*,*) "total area:", total_area
      ...
      do actual_cpu = 0, n_cpu - 2 ! the last process gets the rest
         ...
         do while ((actual_area_sum + area(already_distributed_levels + actual_level_counter)) .lt. total_area / n_cpu)
      ...
      do grid_x = my_start_x, my_start_x + my_n_x_grid
         do grid_y = 1, particles_per_dimension
            do grid_z = 1, particles_per_dimension
               ...
               if (dist_from_center2 .le. sphere_radius2) then ! use this particle
