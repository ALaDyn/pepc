...
contains
   ...
   subroutine write_particles(i, simtime)
      ...
      if ((dump_time .ne. 0) .and. (mod(i, dump_time) .eq. 0)) then

         call write_particles_to_vtk(i, simtime)

      end if

      if ((cp_time .ne. 0) .and. (mod(i, cp_time) .eq. 0)) then
...
!     use utils
      ...
      if (my_rank .eq. 0) write (*, *) "IO: write particles in mpi-io ascii 02 mode. Step:", step
!     flush(6)
         ...
         if (COMMAND_ARGUMENT_COUNT() .ne. 0) then
            ...
            if (argument1(1:7) .eq. '-resume') then
               ...
               if (COMMAND_ARGUMENT_COUNT() .gt. 2) then
                  ...
                  if (output_type .gt. 0 .and. output_type .lt. num_known_output_formats) then
                     call GET_COMMAND_ARGUMENT(3, argument3)
                     data_available = .true.
                     datafile = trim(argument3)
                     run_type = 3
         ...
         if (header(1:14) .eq. '# SPH_ASCII01') then
            ...
            if (input_particles .ne. npart_total) then
               ...
               if (run_type .eq. 3) then ! converting data
                  ...
                  if (np_local * n_cpu .ne. npart_total .and. mod(npart_total, n_cpu) .gt. my_rank) np_local = npart_total / n_cpu + 1
