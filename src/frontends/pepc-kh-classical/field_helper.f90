...
contains
   ...
   subroutine setup_field_grid(field_grid, pepc_comm)
      ...
      if (mpi_rank .lt. mod(field_grid%ntot, mpi_size)) &
   ...
   subroutine compute_field(pepc_pars, field_grid, p)
      ...
      do ipl = 1, size(p)
         ...
         if (any(p(ipl)%x(1:2) .lt. field_grid%offset) .or. &
             any((p(ipl)%x(1:2) - field_grid%offset) .ge. field_grid%extent)) cycle

         ic = ceiling((p(ipl)%x(1:2) - field_grid%offset) / field_grid%dx)

         if (p(ipl)%data%q .lt. 0.) then ! electron
            ...
            if (p(ipl)%label .eq. LABEL_ELECTRON_LEFT) then
            ...
            if (p(ipl)%label .eq. LABEL_ION_LEFT) then
   ...
   contains
      ...
      subroutine write_quantity_on_grid(yname, y)
         ...
         if (pepc_comm%mpi_rank .eq. 0) then
            ...
            if (mpi_disp .gt. field_header_size) then
