...
contains
  ...
  subroutine write_field_grid_params(file_name, field_grid)
    use encap
    implicit none
    ...
    open(param_file_id, file = trim(file_name), status = 'old', position = &
      'append', action = 'write')
    write(param_file_id, nml = field_grid_nml)
    close(param_file_id)
  ...
  subroutine write_field_on_grid(pepc_comm, time_pars, step, physics_pars, field_grid)
    ...
    subroutine write_quantity_on_grid(yname, y)
      ...
      if (pepc_comm%mpi_rank == 0) then
        ...
        call mpi_file_write(fh, time_of_step(step, time_pars), 1, MPI_REAL8, mpi_stat, mpi_err)
