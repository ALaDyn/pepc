...
contains
  ...
  subroutine write_physics_params(file_name, physics_pars)
    use encap
    implicit none
    ...
    open(param_file_id, file = trim(file_name), status = 'old', position = &
      'append', action = 'write')
    write(param_file_id, nml=physics_nml)
    close(param_file_id)
  ...
  subroutine physics_dump(pepc_pars, physics_pars, time_pars, step, p)
    ...
    subroutine write_to_file()
      ...
      open(file_energy, file = 'energy.csv', status = 'unknown', position = 'append', action = 'write')
      write(file_energy, *) time_of_step(step, time_pars), ", ", e_kin_g, ", ", &
