...
program pepc
  ...
  ! timing variables
  real*8 :: timer(5)
  real*8 :: t1, t2, average_walk_time
  ...
  do step=1, nt
    ...
    t2 = get_time()
    if(root) write(*,'(a,F12.3)') " ====== tree grow time  :", t2-t1
    t1 = get_time()
    call pepc_traverse_tree(particles)
    t2 = get_time()
    if(root) write(*,'(a,F12.3)') " ====== tree walk time  :", t2-t1
    if(step .gt. 2) average_walk_time = average_walk_time + (t2-t1)
    ...
    timer(4) = get_time()
    if(root) write(*,'(a, I8, F12.3)') " ====== time in step [s]:", tnp, timer(4) - timer(3) 
  ...
  if(root) then
    write(*,*)            " "
    write(*,'(a,F12.3)') "        FORCE_COMP_TIME =  ", average_walk_time / (nt - 2) 
    write(*,'(a)')        " ===== finished pepc simulation"
    write(*,'(a,F12.3)') " ===== total run time [s]: ", timer(5) - timer(1)
  end if
