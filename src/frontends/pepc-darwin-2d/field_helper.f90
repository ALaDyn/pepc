...
!      call read_in_params(pepc_nml, para_file_available, para_file_name)
   ...
   subroutine setup_field_grid(field_grid, pepc_comm)
      ...
      if (mpi_rank .lt. mod(field_grid%ntot, int(mpi_size, kind=kind_particle))) &
...
!    field_grid%Bbz = zero
      ...
      do ipl = 1, size(p)
         ...
         if (any(p(ipl)%x(1:3) .lt. field_grid%offset(1:3)) .or. &
             any((p(ipl)%x(1:3) - field_grid%offset(1:3)) .ge. field_grid%extent(1:3))) cycle
