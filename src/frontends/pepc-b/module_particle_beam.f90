...
!  if (my_rank==0)   call flvisit_spk_check_connection(lvisit_active)
  if (my_rank==0)   call flvisit_nbody2_check_connection(lvisit_active)
  call MPI_BCAST( lvisit_active, 1, MPI_INTEGER, 0, MPI_COMM_WORLD,ierr)
#endif

  if (lvisit_active==0 )then
     if (my_rank==0) write(*,*) ' No Connection to Visualization'
     return
  endif
...
!  endif
  ...
  if (beam_config >= 3 .and. beam_config <=5 ) then
     ...
     if (u_beam/vosc <10.0 .and. u_beam/vosc > 0.1) then
        ...
        if (my_rank==0 .and. vosc /= vosc_old) write(*,*) 'Laser amplitude changed'
     else
        if (my_rank==0) write(*,*) 'Amplitude change too big - readjust!'
     endif

     if (sigma/rho_beam > 0.1 .and. sigma/rho_beam < 10.0) then
        sigma=rho_beam
        if (my_rank==0 .and. sigma /= sigma_old) write(*,*) 'Laser spot size changed'
     else
        if (my_rank==0) write(*,*) 'Spot size change too big - readjust!'
     endif
     if (tpulse/r_beam > 0.1 .and. tpulse/r_beam < 10.0) then
        tpulse=r_beam
     else
        if (my_rank==0) write(*,*) 'Pulse length change too big - readjust!'
        if (my_rank==0 .and. tpulse /= tpulse_old) write(*,*) 'Laser pulse length changed'
     endif

     if (th_beam /= theta_beam) then
        theta_beam=th_beam
        if (my_rank==0 .and. theta_beam /= theta_old) write(*,*) 'Incidence angle changed'
     endif

  else if (beam_config ==2) then
     nb_pe = np_beam_dt/n_cpu  ! # beam particles to load per PE

     if (my_rank ==0 .and. beam_debug) then
     ...
     if (my_rank==0 .and. u_beam /= u_old) write(*,*) 'Beam velocity changed'
...
!     bond_const = 10**(rho_beam)
     if (my_rank==0) write(*,*) 'Steering pars: a_i=',a_ii,' Ti=',Ti_kev,' Pot strength=',bond_const
