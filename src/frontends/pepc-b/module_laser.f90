...
contains
   ...
   subroutine laser(I_laser)
      ...
      if (beam_config .ge. 3) tlaser = tlaser + dt
      pha = tlaser * omega
      ...
      case (5)  ! propagating fpond
         ...
         if (restart .and. beam_config .eq. 5 .and. focus(1) .ge. window_min + x_plasma * rezone_frac) then
      ...
      case (4, 14, 24, 94)  ! old fpond model
         if (itime .gt. 0) focus(1) = x_crit  ! laser tracks n_c
   ...
   subroutine force_laser(p_start, p_finish)
      ...
      if (itime .gt. 0 .and. beam_config .eq. 4) focus(1) = x_crit  ! laser tracks n_c

      ! Include force from laser/external field on electrons - ES scheme

      if (beam_config .ge. 3 .and. beam_config .le. 7) then
         do p = p_start, p_finish
            ...
            else if (beam_config_in .eq. 27) then !  Tokamak: circular B in theta + const Bz
               xt = x(p) - plasma_centre(1)
               yt = y(p) - plasma_centre(2)
               zt = z(p) - plasma_centre(3)
               rt = sqrt(xt**2 + yt**2)
   ...
   subroutine empond(t, tpulse, sigma0, a0, w0, x, y, z, ez, by, bx, az, phipon)
      ...
      if (r .le. 2.*sigma .and. r .ne. 0.) then
   ...
   subroutine fpond_helm(t, sigma_in, omega, &
      ...
      if (x .lt. xh_start .or. x .gt. xh_end .or. r .ge. 2 * sigma) then
         yf = 0.
         zf = 0.
         Rpon = 0.

      else if (r .lt. 2 * sigma .and. r .ne. 0.) then
         yf = -pi / 4./sigma * y / r * sin(2 * theta)
         zf = -pi / 4./sigma * z / r * sin(2 * theta)
         Rpon = cos(theta)**2
   ...
   subroutine em_helmholtz(me, itime, n, dx, theta, a0, w0, rhoe, Az)
      ...
      iplas = n / 2
      if (me .eq. 0) write (*, '(i6,2f12.3)') itime, rhoe(iplas), abs(az(iplas))
      if (itime .eq. itav .and. me .eq. 0) then
   ...
   subroutine track_nc
      ...
      if (target_geometry .eq. 1 .or. target_geometry .eq. 3) then
      ...
      do while (i .gt. 0 .and. .not. found)
         if (rho1d(i) .le. rho_track .and. rho1d(i + 1) .gt. rho_track) then
            found = .true.
      ...
      !  sweep up
      i = icm
      do while (i .lt. ngx .and. .not. found)
         if (rho1d(i) .le. rho_track .and. rho1d(i + 1) .gt. rho_track) then
            found = .true.
            x_crit = i * dx + dx * (1.-rho1d(i)) / (rho1d(i + 1) - rho1d(i))
            icrit = i
      ...
      if (.not. found .and. itime .gt. 0) then
         beam_config = 0 ! switch off laser
         if (my_rank .eq. 0) write (6, *) 'Target burnt through - switching off laser'
      end if
   ...
   subroutine density_helmholtz
      ...
      if (my_rank .eq. 0 .and. itime .eq. 1) then
