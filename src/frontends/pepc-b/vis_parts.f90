...
subroutine vis_parts
   ...
   do i = 1, npp
      ...
      if ((npart .ge. 100000 .and. (u2 .gt. uthresh .and. q(i) .lt. 0)) .or. (npart .lt. 100000 .and. mod(pelabel(i), nskip) .eq. 0)) then
         nship = nship + 1
         nbufe = nbufe + 1
         !        nbufi=nbufi+1
   ...
   if (npart_buf .gt. 0 .and. npart_buf .lt. 2 * ship_max) then
      ! send  particle data from all PEs
      ! increase momentum threshold if close to max ship #
      if (1.0 * npart_buf / ship_max .gt. 0.9) uthresh = uthresh * 1.1
      if (me .eq. 0) then
      ...
      if (me .eq. 0) then
         if (npart_buf .gt. 2 * ship_max) then
