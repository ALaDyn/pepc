...
program program_pepcb
...
#ifdef VISIT_NBODY
   if (my_rank .eq. 0 .and. vis_on) then
...
#ifdef NETCDFLIB
   if (my_rank .eq. 0 .and. netcdf) then
   ...
   if (my_rank .eq. 0) write (*, *) 'Computing initial fields'

   call pepc_fields_coulomb_wrapper(np_local, x(1:np_local), y(1:np_local), z(1:np_local), &
                                    q(1:np_local), work(1:np_local), pelabel(1:np_local), &
                                    ex(1:np_local), ey(1:np_local), ez(1:np_local), pot(1:np_local), &
                                    itime, .true., force_const)

   ! Centre velocities with 1/2 step back
   call integrator

   ! Static error test mode
   if (np_error .gt. 0) then
   ...
   do itime = 1, nt
      ...
      if (my_rank .eq. 0) then
         ...
         ! laser protocol
         if (debug_level .ge. 2 .and. beam_config .ne. 0) call laser_monitor(I_laser)
      end if
      ...
      if (my_rank .eq. 0 .and. debug_level .ge. 0 .and. mod(itime, iprot) .eq. 0) then
         irecord = irecord + 1
         !        do ifile = 6,15,9
         ifile = 15
