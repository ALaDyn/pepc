...
contains
   ...
   subroutine push_nonrel(ips, ipf, delt)
      ...
      do p = ips, ipf
         ...
         if (p .eq. 1) write (70, *) itime * delt, x(p), y(p), ux(p), uy(p)
      end do
   ...
   subroutine push_x(ips, ipf, delt)
      ...
      do p = ips, ipf
         ...
         dr2 = dx**2 + dy**2 + dz**2
         if (dr2 .lt. r_glue2) then
            gamma = sqrt(1.0 + ux(p)**2 + uy(p)**2 + uz(p)**2)
            x(p) = x(p) + ux(p) / gamma * delt
            if (idim .gt. 1) y(p) = y(p) + uy(p) / gamma * delt
            if (idim .eq. 3) z(p) = z(p) + uz(p) / gamma * delt
         else
   ...
   subroutine velocities(p_start, p_finish, delta_t)
      ...
      case (2)
         ...
         if (my_rank .eq. 0) write (*, *) 'Ti_unc ', Ti_uncor, ' Ti0 ', Ti0, ' chii ', chii
         ...
         do p = 1, np_local
            if (pelabel(p) .le. ne) then
               ...
               if (idim .eq. 3) uz(p) = (2 * chie - 1.) * uz(p) + chie * delta_t * accz(p)

            elseif (pelabel(p) .le. ne + ni) then
               ux(p) = (2 * chii - 1.) * ux(p) + chii * delta_t * accx(p)
               uy(p) = (2 * chii - 1.) * uy(p) + chii * delta_t * accy(p)
               if (idim .eq. 3) uz(p) = (2 * chii - 1.) * uz(p) + chii * delta_t * accz(p)
            end if
         end do
