...
contains
   ...
   subroutine openfiles
      ...
      if (debug_level .gt. 0) then
         if (my_rank .eq. 0) call create_directory("diag")
         call debug_barrier()
   ...
   subroutine closefiles
      ...
      if (debug_level .gt. 0) close (file_ipefile)
   ...
   subroutine predef_parts
      ...
      if (my_rank .eq. 0) then
         ...
         if (ner .ne. ne .or. nir .ne. ni) then
      ...
      if (stopflag) then
         if (my_rank .eq. 0) write (*, *) "*** Stopping program"
      ...
      if (my_rank .eq. 0) write (*, *) "Finished reading particle data"
   ...
   subroutine predef_parts
      ...
      if (my_rank .eq. 0) then
         ...
         if (my_rank .eq. 0) write (*, '(7(a12,i12/),9(a12,f12.5/),9(a12,1pe12.5/),2(a12,3(1pe12.5)/))') &    ! info block - skip variable names
            'Start time: ', itime_start, &
            '# particles: ', npartr, &
            '# electrons: ', ner, &
            '# ions: ', nir, &
      ...
      if (ncpu_merge .lt. 0) then

         ! n_cpu > # data sets, so carve up restart data

         if (ncpu_merge .eq. -1) then
         ...
         if (my_rank .eq. 0) write (*, *) 'Splitting sets by 1:', ncpu_merge
         ...
         if (my_rank .eq. 0) write (*, '(a,a)') 'Reading info file', cfile
         open (60, file=cfile)
         read (60, '(2(9x,i8/))') timestamp, npp_total  ! Find # particles to be read
         close (60)
         if (ncpu_merge .eq. n_cpu) npp_total = ne + ni  ! Override total # if dataset to be split amoung all PEs
         np_local = npp_total / ncpu_merge
         nrest = mod(npp_total, ncpu_merge)  ! Remainder

         if (mod(my_rank, ncpu_merge) .eq. ncpu_merge - 1) then
            nadd = nrest  ! Put remainder on last PE
         else
            nadd = 0
         end if

         if (mod(my_rank, 1000) .eq. 0) write (*, *) 'PE ', my_rank, ': Reading ', np_local + nadd, ' particles out of ', npp_total, ' from ', cme(1:lendir)
         ! Record # particles read in openfiles.out
         ...
         do j = 0, mod(my_rank, ncpu_merge) - 1
            if (debug_level .gt. 0) write (ipefile, *) 'skip pass ', j
            do i = 1, np_local
               read (60, *) x(i), y(i), z(i), ux(i), uy(i), uz(i), q(i), m(i), &
                  axdum, aydum, azdum, phidum, idummy, pelabel(i)
               if (beam_config .eq. 5 .and. q(i) .gt. 0 .and. x(i) .lt. window_min + dt .and. x(i) .gt. window_min) then
                  ! create rezoning slice for wakefield mode - first few blocks should be sufficient
                  nslice = nslice + 1
                  xslice(nslice) = x(i) + x_plasma ! Include offset for new slice
                  yslice(nslice) = y(i)
         ...
         ioffset = 0
         np_local = 0
         if (ncpu_merge .gt. 1) write (*, *) 'Merging sets by ', ncpu_merge, ' : 1'
      ...
      if (T_scale .ne. 1) then
         if (my_rank .eq. 0) write (*, *) 'Rescaling temperature by ', T_scale, ' to ', Te_keV
         do i = 1, np_local
            if (q(i) .lt. 0) then
      ...
      if (my_rank .eq. 0) write (*, *) "Finished reading in particle data"
   ...
   subroutine slices(timestamp)
      ...
      if (my_rank .eq. 0) then
         ...
         Qtot = SUM(rhoe) * dx * dy * dz  ! including ghost cells
         if (debug_level .gt. 1) write (ipefile, '(4(a,f14.5/))') &
            'Total charge on grid:', Qbox, &
            '         ghost cells:', Qtot - Qbox, &
            '                 sum:', Qtot, &
