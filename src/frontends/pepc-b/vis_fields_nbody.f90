...
subroutine vis_fields_nbody(timestamp)
   ...
   if (lvisit_active .eq. 0) then
      if (me .eq. 0) write (*, '(a)') 'VIS_FIELDS  | No connection to visualization'
   end if
   ...
   if (me .eq. 0) write (*, '(a,4i6)') 'VIS_FIELDS  | Selection:', fselect1, fselect2, fselect3, fselect4
