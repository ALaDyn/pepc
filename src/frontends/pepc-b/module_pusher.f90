...
contains
   ...
   subroutine push_x(ips, ipf, delt)
      ...
      do p = ips, ipf
         ...
         if (idim .gt. 1) y(p) = y(p) + uy(p) / gam * delt
         if (idim .eq. 3) z(p) = z(p) + uz(p) / gam * delt
      end do
