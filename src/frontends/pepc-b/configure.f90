...
subroutine configure
   ...
   if (restart) then
      ...
      if (beam_config .eq. 4) call track_nc
      if (beam_config .eq. 5) focus(1) = focus(1) + x_offset
      ...
      case (1)        ! Set up single neutral plasma target according to geometry
         ...
         if (debug_level .eq. 2 .and. my_rank .eq. 0) then
...
!            target_geometry=1
      ...
      case (6)        ! Spherical cluster with Andreev profile
         ...
         if (debug_level .ge. 1 .and. my_rank .eq. 0) then
...
!            target_geometry=0
   ...
   if (launch) then ! Ignore if still in config loop
...
#endif
         if (my_rank .eq. 0) write (*, '(//a)') '===> Particle beam switched on'
      ...
      case (3:6) ! laser on
         ...
         if (my_rank .eq. 0) write (*, '(//a)') '===> Laser switched on'
#ifdef VISIT_NBODY
