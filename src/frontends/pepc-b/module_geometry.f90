...
contains
   ...
   subroutine plasma_start(my_rank, i1, n, nglobal, label_off, target_geometry, idim, &
      ...
      if (idim .eq. 3 .or. idim .eq. 1) then    ! 3D
...
!        uz(i) = v0/10.
      ...
      case (7)       ! regular ion grid for quiet start in x,y plane
         ...
         if (my_rank .eq. 0) write (*, *) 'PEPC-B | Special start 7 - ion grid: ', Aplas, dx, ncols, nrows
         if (my_rank .eq. 0 .and. dx .ne. a_ii) write (*, *) 'PEPC-B | non-integer a_ii', a_ii, dx
...
!        write (*,*) p,i,j,x(p),y(p)
         ...
         do p = 1, nep
            ...
            if (idim .eq. 3) zt = .1 * a_ii * (2 * rano(iseed) - 1.)
      ...
      case (8)       ! capacitor plates with quiet start in x,y plane (as plasma config 3)
         ...
         ncols = nint(x_plasma / dx)
         nrows = nint(y_plasma / dx)
         if (my_rank .eq. 0) write (*, *) 'PEPC-B | Special start 8 - ion grid: ', Aplas, dx, ncols, nrows
         if (my_rank .eq. 0 .and. dx .ne. a_ii) write (*, *) 'PEPC-B | non-integer a_ii', a_ii, dx
