...
subroutine vis_parts_nbody
  ...
  real, dimension(0:attrib_max-1,npart_visit_max) :: vbuffer
  real, dimension(0:attrib_max-1,nppm) :: vbuf_local
...
! Gather particle data onto root PE
     ...
     call MPI_GATHERV( vbuf_local, nship*attrib_max, MPI_REAL8, vbuffer, nbuf_pe, recv_strides, MPI_REAL8, 0, MPI_COMM_WORLD, ierr )

 
! Ship to visualization
     if (me.eq.0) then
        call flvisit_nbody2_partstep_send(vbuffer,nship,attrib_max)
     endif
