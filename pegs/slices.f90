...
subroutine slices(timestamp)
  ...
  if (me==0) then
     ...
     cfile = "fields/slice."//cdump
     open (62,file=cfile)

     jfoc = box_centre(2)/dy
     kfoc = box_centre(3)/dz

     ! density average line-out along laser axis: 5x5 average

     work1 = 0.
     work2 = 0.
     do k=kfoc-2,kfoc+2
        do j=jfoc-2,jfoc+2
           work1(1:ngx) = work1(1:ngx)+rhoi(1:ngx,j,k)/25.  ! density slice along laser axis: 5x5 average
           work2(1:ngx) = work2(1:ngx)+rhoe(1:ngx,j,k)/25.
        end do
     end do
