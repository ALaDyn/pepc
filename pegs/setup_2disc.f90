...
subroutine setup
  ...
  if (me == 0) then 
     ...
     !     write(*,'(4(a9,f12.2))') "xl ",xl,"  yl ",yl,"  zl ",zl,"  boxsize ",boxsize
  ...
  if(ndisc.eq.2) then
     do i=1,npp/2
        x(i+npp/2)=x(i)+x_star(2)
        y(i+npp/2)=y(i)+y_star(2)
        z(i+npp/2)=z(i)+z_star(2)
        ux(i+npp/2)=ux(i)+ux_star(2)
        uy(i+npp/2)=uy(i)+uy_star(2)
        uz(i+npp/2)=uz(i)+uz_star(2)
        q(i+npp/2)=q(i)
        m(i+npp/2)=-m(i) ! q is used instead of m in PEPC - use m as disc label instead
     end do
  endif

  !  Derive some constants from input data

  box_centre =  (/ xl/4.d0, 3.d0*yl/4.d0, zl/2.d0 /) ! Centre of box
