...
subroutine forces(p_start,p_finish,delta_t, t_walk, t_force)
  ...
  integer :: p, i, j, npass, jpass, ip1, nps,  max_npass,nshort_list, ipe, ierr
  ...
  do jpass = 1,max_npass
     ...
     do i = 1, nps
        ...
        pot(p) = pot(p) - gamma * phi_coul
        fx(p) = fx(p) - gamma * ex_coul
        fy(p) = fy(p) - gamma * ey_coul
        fz(p) = fz(p) - gamma * ez_coul
  ...
  ! forces from stars on dust particles
     ...
     do p = p_start, p_finish     
      do j=1,ni
       ...
       fx(p)=fx(p)-gamma*m_star(j)*distx/(dists**3)
       fy(p)=fy(p)-gamma*m_star(j)*disty/(dists**3)
       fz(p)=fz(p)-gamma*m_star(j)*distz/(dists**3)
