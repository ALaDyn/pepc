...
!!! --------------- TREE BUILD ---------------
  ...
  tree_nodes(-1)%leaves = npp              ! root contains all leaves, excluding boundary particles
  ntwig = 1
  nleaf = 0
  ...
  do while ( nlist > 0 )              ! While any particle not finished:
     ...
     do i = 1,nlist
        if ( htable( cell_addr(i) )%node == 0 .and. htable(cell_addr(i))%key /= -1 ) then       ! Is entry empty?
           ...
           tree_nodes(i)%leaves = 1                !   # children
           ...
           tree_nodes(i)%leaves = tree_nodes(i)%leaves + 1
     ...
     do i = 1,ncoll
        ...
        if ( htable( res_addr(i) )%link == -1) then     ! Entry occupied without link
           ...
           tree_nodes( res_node(i) )%leaves = res_child(i)
           ...
           do while ( .not. resolved )       ! Find end of chain  
              ...
              if ( htable( link_addr )%key == res_key(i) ) then
                 ...
                 tree_nodes(htable( link_addr )%node)%leaves =  tree_nodes(htable( link_addr )%node)%leaves + 1
                 resolved = .true.
     ...
     do i = 1,nres
        ...
        if ( tree_nodes(htable( newentry(i) )%node)%leaves == 1 .and. htable(newentry(i))%owner == me) then
           ! create new leaf
           newleaf = newleaf + 1 
           nleaf = nleaf + 1 
           ipoint =  htable( newentry(i) )%node                  ! local pointer
           tree_nodes( ipoint )%leaves = 1                       ! contained leaves
        ...
        else if ( tree_nodes(htable( newentry(i) )%node)%leaves == 1 .and. htable(newentry(i))%owner /= me) then
           ...
           tree_nodes( ipoint )%leaves = 0
           htable( newentry(i) )%childcode = 0 
        else if ( tree_nodes(htable( newentry(i) )%node)%leaves > 1 ) then
           ! twig
