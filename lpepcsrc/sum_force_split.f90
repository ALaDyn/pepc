...
subroutine sum_force_split( p, n, inode, eps, sumfx, sumfy, sumfz, sumphi, load )
  use treevars
  implicit none
  ...
  do j=1,n
     ...
     if (jnode>0) then
        ...
        k = (nq-1)*3+1
        xquad(k) = xcoc(jnode)
        xquad(k+1) = ycoc(jnode)
        xquad(k+2) = zcoc(jnode)
        mult(i) = charge(jnode)
        mult(i+1) = xdip(jnode)
        mult(i+2) = ydip(jnode)
        mult(i+3) = zdip(jnode)
...
end subroutine sum_force_split
