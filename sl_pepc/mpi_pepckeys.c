...
void slsort_keys_(finteger_t *, finteger_t *, pepckeys_slkey_t *, pepckeys_sldata0_t *, finteger_t *, double *, finteger_t *, finteger_t *, finteger_t *,
                  finteger_t *, finteger_t *, finteger_t *, finteger_t *, pepckeys_slkey_t *, pepckeys_sldata0_t *, finteger_t *, finteger_t *);
...
#ifdef TIMING
  double ttotal, tinitpre, tpresort, tpartition, talltoall, tinitpost, tpostsort;
#endif
...
/*  pepckeys_mpi_partition_radix(&s0, &pc, 59, 15, 3, scounts, sdispls, size, rank, comm);*/
  TSTOP(tpartition);
  ...
  *nout = rdispls[size - 1] + rcounts[size - 1];
  ...
  for (i = 0; i < s0.size; ++i) irnkl[irnkl2[i] - 1] = i + 1;
...
#ifdef VALIDATE
  o = pepckeys_mpi_elements_validate_order(&s0, 1, size, rank, comm);
 #ifdef VERBOSE
  ...
  if (rank == 0)
  {
    ...
    printf("%d: slsort_keys: postsort: %f\n", rank, tpostsort);
  }
#endif
}
