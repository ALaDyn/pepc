 ...
 *  
 *  file: src/core_mpi/pepckeys_mpi_merge2.c
 *  timestamp: 2009-11-26 17:36:13 +0100
 *  
 */
...
slint_t pepckeys_mpi_merge2(elements_t *s, slint_t other_rank, slint_t high_rank, slint_t *dst_size, merge2x_f m2, elements_t *xs, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_merge2 */
{
  const int tag = 1;
...
#ifdef CHECK_ORDER
  check_order = pepckeys_elements_validate_order(s, 1);
  if (check_order) fprintf(stderr, "%d: pepckeys_mpi_merge2: warning: input order failed (at %" sl_int_type_fmt ")\n", rank, check_order);
#endif

  rti_tstart(rti_tid_mpi_merge2_fe);
  pepckeys_mpi_find_exact(s, other_rank, high_rank, dst_size, &ex_start, ex_sizes, &nx_move, size, rank, comm);
...
#ifdef CHECK_ORDER
  check_order = pepckeys_elements_validate_order(&s0, 1);
  if (check_order) fprintf(stderr, "%d: pepckeys_mpi_merge2: warning: intermediate lower order failed (at %" sl_int_type_fmt ")\n", rank, check_order);
  check_order = pepckeys_elements_validate_order(&s1, 1);
  if (check_order) fprintf(stderr, "%d: pepckeys_mpi_merge2: warning: intermediate higher order failed (at %" sl_int_type_fmt ")\n", rank, check_order);
#endif
...
#ifdef CHECK_ORDER
  check_order = pepckeys_elements_validate_order(s, 1);
  if (check_order) fprintf(stderr, "%d: pepckeys_mpi_merge2: warning: output order failed (at %" sl_int_type_fmt ")\n", rank, check_order);
#endif
