...
slint pepckeys_mpi_sample_ci_init(classification_info *ci, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_sample_ci_init */
{
  ci->nclasses = 0;
  ci->keys = NULL;
  ci->counts = NULL;
...
slint pepckeys_mpi_sample_ci_duplicate(classification_info *ci_src, classification_info *ci_dup, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_sample_ci_duplicate */
{
  pepckeys_mpi_sample_ci_init(ci_dup, size, rank, comm);
...
slint pepckeys_mpi_sample_ci_free(classification_info *ci, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_sample_ci_free */
{
...
slint pepckeys_mpi_sample_ci_print(classification_info *ci, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_sample_ci_print */
{
  slint i;
