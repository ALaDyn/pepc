 ...
 *  
 *  file: src/core_mpi/pepckeys_mpi_find_exact_old.c
...
slint_t pepckeys_mpi_find_exact_old(elements_t *s, slint_t counterpart, slint_t high, elements_t *xs, slint_t *start, slint_t *end, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_find_exact_old */
{
  elements_t x;
  ...
  {
    xs = &x;
    pepckeys_elements_alloc(xs, 1, 1, 0);
  }
...
/*    printf("%d here: intervall", rank); pepckeys_elements_printf(&intervall);
...
/*  printf("%d here: finished with ", rank); pepckeys_elements_printf(&intervall);*/
  ...
  if (xs == &x) pepckeys_elements_free(xs);
