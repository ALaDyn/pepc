...
slint pepckeys_merge2_basic_01_x(elements_t *s0, elements_t *s1, elements_t *sx, m2x_func _x0_1, m2x_func _0x_1) /* pepckeys_sl_proto, sl_func pepckeys_merge2_basic_01_x */
{
  elements_t s, e, d, x;
  ...
  if (sx == NULL)
  {
    pepckeys_elements_alloc(&x, xmin(s0->size, s1->size), 1, 1);
    sx = &x;
  ...
  if (sx == &x) pepckeys_elements_free(&x);

  return 0;
}


slint pepckeys_merge2_basic_01_X(elements_t *s0, elements_t *s1, elements_t *sx, elements_t *t, m2X_func _X0_1, m2X_func _0X_1) /* pepckeys_sl_proto, sl_func pepckeys_merge2_basic_01_X */
{
  elements_t s, e, d, x;
  ...
  if (s0->size <= s1->size)
  {
    ...
    /* evacuate wrong-placed elements of s0 */
    d.size = s.size = e.keys - s.keys;
    pepckeys_elem_nxchange_ro0(&s, &d, d.size, t);
  ...
  } else
  {
    ...
    d.size = e.size = s.keys - e.keys;
    elem_inc(&e);
    pepckeys_elem_nxchange_ro0(s1, &d, d.size, t);
