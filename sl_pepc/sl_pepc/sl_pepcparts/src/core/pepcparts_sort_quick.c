 ...
 *  
 *  file: src/core/pepcparts_sort_quick.c
...
slint pepcparts_qs_halfrec_i(elements_t *s, slint l, slint h, elements_t *xs) /* sl_func pepcparts_qs_halfrec_i */
{
  slint q, i, j;
  ...
  while (l < h)
  {
    ...
    {
      q--;
      if (l < q) pepcparts_qs_halfrec_i(s, l, q, xs);
      l = q + 2;

    } else
    {
      q++;
      if (q < h) pepcparts_qs_halfrec_i(s, q, h, xs);
      h = q - 2;
    }
  }
...
slint pepcparts_sort_quick_i(elements_t *s, elements_t *xs) /* sl_func pepcparts_sort_quick_i */
{
  elements_t txs;
  ...
  pepcparts_qs_halfrec_i(s, 0, s->size - 1, xs);
...
slint pepcparts_qs_halfrec_p(elements_t xl, elements_t xh, elements_t *xs) /* sl_func pepcparts_qs_halfrec_p */
{
  elements_t xi, xj;
  ...
  while (xl.keys < xh.keys)
  {
    ...
    {
      elem_dec(&xi);
      if (xl.keys < xi.keys) pepcparts_qs_halfrec_p(xl, xi, xs);
      elem_assign_at(&xi, 2, &xl);

    } else
    {
      elem_inc(&xi);
      if (xi.keys < xh.keys) pepcparts_qs_halfrec_p(xi, xh, xs);
...
slint pepcparts_sort_quick_p(elements_t *s, elements_t *xs) /* sl_func pepcparts_sort_quick_p */
{
  elements_t xh, txs;
  ...
  if (s->size > 1)
  {
    ...
    pepcparts_qs_halfrec_p(*s, xh, xs);
  }
...
slint pepcparts_qs_hybrid(elements_t xl, elements_t xh, elements_t *xs) /* sl_func pepcparts_qs_hybrid */
{
  elements_t xi, xj;
...
#define pop(l, h)   ((sp > stack)?(--sp, l = sp->xl, h = sp->xh, 1):0)
  ...
  while (1)
  {
    ...
    if (xi.keys - xl.keys < xh.keys - xi.keys)
    {
      ...
      if (xl.keys < xi.keys)
      if (!push(xl, xi)) pepcparts_qs_hybrid(xl, xi, xs); /* call if push fails */
    ...
    } else
    {
      ...
      if (xi.keys < xh.keys)
      if (!push(xi, xh)) pepcparts_qs_hybrid(xi, xh, xs); /* call if push fails */
...
slint pepcparts_sort_quick_h(elements_t *s, elements_t *xs) /* sl_func pepcparts_sort_quick_h */
{
  elements_t xh, txs;
...
slint pepcparts_sort_quick(elements_t *s, elements_t *xs) /* pepcparts_sl_proto, sl_func pepcparts_sort_quick */
{
  return pepcparts_sort_quick_p(s, xs);
}
