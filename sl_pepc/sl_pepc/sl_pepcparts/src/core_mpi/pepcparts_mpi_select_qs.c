 ...
 *  
 *  file: src/core_mpi/pepcparts_mpi_select_qs.c
...
slint pepcparts_mpi_select_qs(elements_t *s, slint n, slint *iths, pivot_func pi, slint threshold, slkey_pure_t *e, int size, int rank, MPI_Comm comm) /* pepcparts_sl_proto, sl_func pepcparts_mpi_select_qs */
{
...
/*  char outputname[128];
  ...
  MPI_Bcast(&pivots[0], 1, pepcparts_key_pure_mpi_datatype, first_pivot_root, comm);
  for (i = 1; i < n; ++i) pivots[i] = pivots[0];
  
/*  pepcparts_elements_printf(s);
  pepcparts_elements_print_keys(s);*/
...
/*        if (rank == 0)
        {
          ...
          pepcparts_elements_print_keys(&areas[i]);
        }*/

        pepcparts_split3_lt_eq_gt(&areas[i], &pivots[i], &t, &nlt, &nle);
...
/*    MPI_Allreduce(local_partition_sizes, global_partition_sizes, 2 * n, sl_int_type_mpi, MPI_SUM, comm);*/

    /* distribution and local sumation */
    MPI_Allgather(local_partition_sizes, 2 * n, sl_int_type_mpi, all_local_partition_sizes, 2 * n, sl_int_type_mpi, comm);
    for (i = 0; i < n; i++)
    {
...
/*      elem_copy(&areas[i], &t);
    ...
    MPI_Allgather(pivot_proposals, n, pepcparts_key_pure_mpi_datatype, all_pivot_proposals, n, pepcparts_key_pure_mpi_datatype, comm);
