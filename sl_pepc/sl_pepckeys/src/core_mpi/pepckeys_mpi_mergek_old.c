 ...
 *  
 *  file: src/core_mpi/pepckeys_mpi_mergek_old.c
...
slint pepckeys_mpi_mergek_old(elements_t *s, sn_func sn, void *snp, m2x_func m2, elements_t *xs, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_mergek_old */
{
  slint stage = 0, cp, up;
  ...
  while ((cp = (sn)(size, rank, stage++, snp, &up)) >= 0)
  {
    ...
    local_error = pepckeys_elements_validate_order(s, 1);
    ...
    pepckeys_mpi_merge2_old(s, cp, (up)?(xmin(rank, cp)):(xmax(rank,cp)), m2, xs, size, rank, comm);
