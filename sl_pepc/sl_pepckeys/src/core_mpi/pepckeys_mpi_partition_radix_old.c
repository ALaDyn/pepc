 ...
 *  
 *  file: src/core_mpi/pepckeys_mpi_partition_radix_old.c
...
slint_t pepckeys_mpi_partition_radix_old(elements_t *s, partcond_t *pc, slint_t rhigh, slint_t rlow, slint_t rwidth, int *scounts, int *sdispls, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_partition_radix_old */
{
...
/*    if (rank == 0)
    {
      printf("%d: pepckeys_mpi_partition_radix: allreduce: %f (nareas: %" sl_int_type_fmt ", nclasses: %" sl_key_type_fmt " -> %" sl_int_type_fmt " doubles)\n",
        rank,
