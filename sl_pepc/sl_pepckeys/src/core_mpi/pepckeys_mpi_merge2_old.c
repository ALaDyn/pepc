 ...
 *  
 *  file: src/core_mpi/pepckeys_mpi_merge2_old.c
...
slint pepckeys_mpi_merge2_old(elements_t *s, slint counterpart, slint high, m2x_func m2, elements_t *xs, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_merge2_old */
{
...
#ifdef VALIDATE_ORDER
  round++;
  if (o0 = pepckeys_elements_validate_order(s, 1)) printf("%d here: round = %d - input is unsorted (%d)\n", rank, round, o0);
#endif

  rti_tstart(rti_tid_mpi_merge2);

  rti_tstart(rti_tid_mpi_merge2_fe);
  xa = pepckeys_mpi_find_exact_old(s, counterpart, high, xs, &xa_start, &xa_end, size, rank, comm);
...
/*    printf("sending %d from %d to %d DONE (%d == %d?)\n", xa, rank, counterpart, mpi_error, MPI_SUCCESS);*/
    ...
    if (xa < s->size)
    {
      ...
      if (rank == high) pepckeys_elements_extract(s, xa, &s0, &s1);
      else pepckeys_elements_extract(s, s->size - xa, &s0, &s1);

#ifdef VALIDATE_ORDER
      if (o0 = pepckeys_elements_validate_order(&s0, 1)) printf("%d here: round = %d - merge2_input_s0 is unsorted (%d)\n", rank, round, o0);
      if (o1 = pepckeys_elements_validate_order(&s1, 1)) printf("%d here: round = %d - merge2_input_s1 is unsorted (%d)\n", rank, round, o1);
#endif

#ifdef TRACE_MERGE2_INOUT
      pepckeys_elements_alloc(&backup, s->size, 1, 1);
...
#ifdef VALIDATE_ORDER
      if (of = pepckeys_elements_validate_order(&s0, 1))
      {
