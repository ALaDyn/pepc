 ...
 *  
 *  file: src/core_mpi/pepckeys_mpi_mergek.c
...
slint_t pepckeys_mpi_mergek_equal(elements_t *s, sortnet_f sn, sortnet_data_t snd, merge2x_f m2x, elements_t *xs, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_mergek_equal */
{
  ...
  {
#ifdef CHECK_ORDER
    local_order = pepckeys_elements_validate_order(s, 1);
    MPI_Allreduce(&local_order, &global_order, 1, sl_int_type_mpi, MPI_SUM, comm);
    if (global_order)
    {
      fprintf(stderr, "%d: pepckeys_mpi_mergek_equal: warning: input order failed (at %" sl_int_type_fmt ")\n", rank, local_order);
...
# endif
    ...
    pepckeys_mpi_merge2(s, other_rank, high_rank, NULL, m2x, xs, size, rank, comm);
    ...
    if (global_order)
    {
      fprintf(stderr, "%d: pepckeys_mpi_mergek_equal: warning: output order failed (at %" sl_int_type_fmt ")\n", rank, local_order);
# ifdef CHECK_ORDER_BREAK
      break;
# endif
    }
...
slint_t pepckeys_mpi_mergek(elements_t *s, sortnet_f sn, sortnet_data_t snd, merge2x_f m2x, elements_t *xs, int size, int rank, MPI_Comm comm) /* pepckeys_sl_proto, sl_func pepckeys_mpi_mergek */
{
  int i;
