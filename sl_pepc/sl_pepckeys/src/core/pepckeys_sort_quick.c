 ...
 *  
 *  file: src/core/pepckeys_sort_quick.c
...
slint pepckeys_qs_halfrec_i(elements_t *s, slint l, slint h, elements_t *xs) /* sl_func pepckeys_qs_halfrec_i */
{
  slint q, i, j;
  ...
  while (l < h)
  {
    ...
    {
      q--;
      if (l < q) pepckeys_qs_halfrec_i(s, l, q, xs);
      l = q + 2;

    } else
    {
      q++;
      if (q < h) pepckeys_qs_halfrec_i(s, q, h, xs);
      h = q - 2;
    }
  }
...
slint pepckeys_sort_quick_i(elements_t *s, elements_t *xs) /* sl_func pepckeys_sort_quick_i */
{
  elements_t txs;
  ...
  pepckeys_qs_halfrec_i(s, 0, s->size - 1, xs);
...
slint pepckeys_qs_halfrec_p(elements_t xl, elements_t xh, elements_t *xs) /* sl_func pepckeys_qs_halfrec_p */
{
  elements_t xi, xj;
  ...
  while (xl.keys < xh.keys)
  {
    ...
    {
      elem_dec(&xi);
      if (xl.keys < xi.keys) pepckeys_qs_halfrec_p(xl, xi, xs);
      elem_assign_at(&xi, 2, &xl);

    } else
    {
      elem_inc(&xi);
      if (xi.keys < xh.keys) pepckeys_qs_halfrec_p(xi, xh, xs);
...
slint pepckeys_sort_quick_p(elements_t *s, elements_t *xs) /* sl_func pepckeys_sort_quick_p */
{
  elements_t xh, txs;
...
slint pepckeys_qs_hybrid(elements_t xl, elements_t xh, elements_t *xs) /* sl_func pepckeys_qs_hybrid */
{
  elements_t xi, xj;
...
#define pop(l, h)   ((sp > stack)?(--sp, l = sp->xl, h = sp->xh, 1):0)
  ...
  while (1)
  {
    ...
    if (xi.keys - xl.keys < xh.keys - xi.keys)
    {
      ...
      if (xl.keys < xi.keys)
      if (!push(xl, xi)) pepckeys_qs_hybrid(xl, xi, xs); /* call if push fails */
    ...
    } else
    {
      ...
      if (xi.keys < xh.keys)
      if (!push(xi, xh)) pepckeys_qs_hybrid(xi, xh, xs); /* call if push fails */
...
slint pepckeys_sort_quick_h(elements_t *s, elements_t *xs) /* sl_func pepckeys_sort_quick_h */
{
  elements_t xh, txs;
...
slint pepckeys_sort_quick(elements_t *s, elements_t *xs) /* pepckeys_sl_proto, sl_func pepckeys_sort_quick */
{
  return pepckeys_sort_quick_p(s, xs);
}
