...
#define the_merge2_01(s0, s1, sx)  pepckeys_merge2_basic_sseq_01(s0, s1, sx)
...
slint pepckeys_merge2_simplify_s1(elements_t *s0, elements_t *s1, elements_t *sx, slint s1elements) /* pepckeys_sl_proto, sl_func pepckeys_merge2_simplify_s1 */
{
  slint m, m0, m1;
...
/*  printf("highest %d elements, %d from s0, %d from s1\n", s1elements, m0, m1);*/
  ...
  pepckeys_elem_nxchange(&_s0, &x, m0, sx);
...
/*  pepckeys_elements_print_keys(s1);*/
...
/*  pepckeys_elements_print_keys(s0);*/
