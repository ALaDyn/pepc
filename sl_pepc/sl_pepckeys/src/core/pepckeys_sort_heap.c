 ...
 *  
 *  file: src/core/pepckeys_sort_heap.c
...
inline slint pepckeys_hs_heapify_i(elements_t *s, slint i, slint size, elements_t *xs) /* sl_func pepckeys_hs_heapify_i */
{
  slint l, r, largest;
...
slint pepckeys_sort_heap_i(elements_t *s, elements_t *xs) /* sl_func pepckeys_sort_heap_i */
{
  slint i;
  elements_t txs;
  ...
  /* build the heap */
  for (i = s->size / 2 - 1; i >= 0; --i) pepckeys_hs_heapify_i(s, i, s->size, xs);

  /* extract the maxima */
  for (i = s->size - 1; i > 0; --i)
  {
    elem_xchange_at(s, 0, s, i, xs);
    pepckeys_hs_heapify_i(s, 0, i, xs);
  }
...
slint pepckeys_sort_heap_p(elements_t *s, elements_t *xs) /* sl_func pepckeys_sort_heap_p */
{
...
slint pepckeys_sort_heap(elements_t *s, elements_t *xs) /* pepckeys_sl_proto, sl_func pepckeys_sort_heap */
{
  return pepckeys_sort_heap_p(s, xs);
}
