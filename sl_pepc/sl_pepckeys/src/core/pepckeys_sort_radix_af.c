 ...
 *  
 *  file: src/core/pepckeys_sort_radix_af.c
...
/* insert sort */
slint pepckeys_rs_rec_insertsort_af(elements_t *s, elements_t *sx, slint rhigh, slint rlow) /* sl_func pepckeys_rs_rec_insertsort_af */
{
  slint i, j;
...
slint pepckeys_rs_rec_af(elements_t *s, elements_t *sx, slint rhigh, slint rlow, slint rwidth, slint *finalize) /* sl_func pepckeys_rs_rec_af */
{
#define max_nclasses (powof2_typed(sort_radix_width_max, slkey_pure_t))
  ...
  if (rhigh >= rlow)
  {
    ...
    for (i = 0; i < nclasses; i++)
    {
...
#ifdef insertsort
      if (xi.size > sort_radix_threshold_rec) pepckeys_rs_rec_af(&xi, sx, rhigh, rlow, rwidth, finalize);
      ...
      else if (xi.size > 1) pepckeys_rs_rec_insertsort_af(&xi, sx, rhigh, rlow);
...
slint pepckeys_sort_radix_af(elements_t *s, elements_t *sx, slint rhigh, slint rlow, slint rwidth) /* pepckeys_sl_proto, sl_func pepckeys_sort_radix_af */
{
  elements_t _sx;
  ...
  pepckeys_rs_rec_af(s, sx, rhigh, rlow, xmin(rwidth, sort_radix_width_max), &finalize);

#ifdef insertsort_finalize
  if (sort_radix_threshold_rec > 1 && finalize) pepckeys_rs_rec_insertsort_af(s, sx, rhigh, rlow);
#endif /* insertsort_finalize */
