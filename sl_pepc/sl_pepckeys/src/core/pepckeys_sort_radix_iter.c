 ...
 *  
 *  file: src/core/pepckeys_sort_radix_iter.c
...
/* configure tuneable */
 ...
 int pepckeys_tuneable_sort_radix_threshold_iter = sort_radix_threshold_iter; /* sl_var pepckeys_tuneable_sort_radix_threshold_iter */
 #undef sort_radix_threshold_iter
 #define sort_radix_threshold_iter pepckeys_tuneable_sort_radix_threshold_iter
...
/* insert sort */
slint pepckeys_rs_iter_insertsort(elements_t *s, elements_t *sx, slint presorted, slint rhigh, slint rlow) /* sl_func pepckeys_rs_iter_insertsort */
{
  slint i, j;
...
slint pepckeys_rs_iter(elements_t *s, elements_t *sx, slint presorted, slint rhigh, slint rlow, slint rwidth) /* sl_func pepckeys_rs_iter */
{
  ...
  while (rhigh >= rlow)
  {
    ...
    while (xi.keys < end.keys)
    {
        ...
        else
      {
        ...
        pepckeys_rs_iter_insertsort(&xj, sx, presorted, rhigh_old, rlow);
      }
 #endif
#endif
    }
...
slint pepckeys_sort_radix_iter(elements_t *s, elements_t *sx, slint presorted, slint rhigh, slint rlow, slint rwidth) /* pepckeys_sl_proto, sl_func pepckeys_sort_radix_iter */
{
  elements_t _sx;
  ...
  pepckeys_rs_iter(s, sx, presorted, rhigh, rlow, xmin(rwidth, sort_radix_width_max));

#ifdef insertsort
 #ifdef insertsort_finalize
  if (sort_radix_threshold_iter > 1) pepckeys_rs_iter_insertsort(s, sx, presorted, rhigh, rlow);
 #endif
#endif
