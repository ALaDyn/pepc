 ...
 *  
 *  file: src/core/pepcparts_merge2_compo_hula.c
...
#define sort_again(s, sx)    pepcparts_sort_radix(s, sx, -1, -1, -1);

#define the_merge2_01(s0, s1, sx)       pepcparts_merge2_basic_sbin_01(s0, s1, sx)
#define the_merge2_0X_1(s0, s1, sx, t)  pepcparts_merge2_basic_straight_0X_1(s0, s1, sx, t)
#define the_merge2_X0_1(s0, s1, sx, t)  pepcparts_merge2_basic_straight_X0_1(s0, s1, sx, t)


slint pepcparts_merge2_compo_hula_(elements_t *s0, elements_t *s1, elements_t *sx) /* sl_func pepcparts_merge2_compo_hula_ */
{
  slint i, j, k;

  slint n0, n1, t0, t1, e0, e1;

  elements_t wa, _src0, _src1, _sx, undersized;
...
/*    printf("easier merge2 done\n");*/
  ...
  pepcparts_merge2_simplify_s1(&_s0, &_s1, sx, s1->size % s);

/*  printf("_s0:\n"); pepcparts_elements_printf(&_s0); pepcparts_elements_print_keys(&_s0);
  printf("_s1:\n"); pepcparts_elements_printf(&_s1); pepcparts_elements_print_keys(&_s1);*/
...
/*  printf("sd:"); pepcparts_elements_printf(&sd);
  printf("sc:"); pepcparts_elements_printf(&sc);
  printf("sb:"); pepcparts_elements_printf(&sb);*/
...
/*  printf("wa:\n"); pepcparts_elements_printf(&wa); pepcparts_elements_print_keys(&wa);*/
...
/*    printf("sg:"); pepcparts_elements_printf(&sg);
    printf("sf:"); pepcparts_elements_printf(&sf);
    printf("x:"); pepcparts_elements_printf(&x);*/
...
/*    printf("_s0:\n"); pepcparts_elements_printf(&_s0); pepcparts_elements_print_keys(&_s0);
    ...
    pepcparts_elem_nxchange(&sf, &x, s0mod, sx);
...
/*    printf("_s0:\n"); pepcparts_elements_printf(&_s0); pepcparts_elements_print_keys(&_s0);
  ...
  pepcparts_elem_nxchange_ro0(&_s0, &wa, s, sx);
...
/*    printf("first %d @ %p\n", *first.keys, first.keys);
    ...
    if (first.keys != lowest_head.keys) pepcparts_elem_nxchange(&first, &lowest_head, s, sx);
...
/*    printf("merging blocks:\n");
    printf("first:\n"); pepcparts_elements_printf(&first); pepcparts_elements_print_keys(&first);
    printf("second:\n"); pepcparts_elements_printf(&second); pepcparts_elements_print_keys(&second);
    printf("wa:\n"); pepcparts_elements_printf(&wa); pepcparts_elements_print_keys(&wa);*/

    /* blockmerge */
    u = pepcparts_merge2_basic_straight_X0_1u(&first, &second, &wa, sx);
...
/*    printf("after merging blocks:\n");
  ...
  pepcparts_elem_nxchange_ro0(&first, &wa, first.size, sx);
...
slint pepcparts_merge2_compo_hula(elements_t *s0, elements_t *s1, elements_t *xs) /* pepcparts_sl_proto, sl_func pepcparts_merge2_compo_hula */
{
  elements_t txs;
  ...
  pepcparts_merge2_compo_hula_(s0, s1, xs);
