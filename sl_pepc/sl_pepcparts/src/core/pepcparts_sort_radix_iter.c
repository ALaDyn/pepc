 ...
 *  
 *  file: src/core/pepcparts_sort_radix_iter.c
...
/* configure tuneable */
 ...
 int pepcparts_tuneable_sort_radix_threshold_iter = sort_radix_threshold_iter; /* sl_var pepcparts_tuneable_sort_radix_threshold_iter */
 #undef sort_radix_threshold_iter
 #define sort_radix_threshold_iter pepcparts_tuneable_sort_radix_threshold_iter
...
/* insert sort */
slint pepcparts_rs_iter_insertsort(elements_t *s, elements_t *sx, slint presorted, slint rhigh, slint rlow) /* sl_func pepcparts_rs_iter_insertsort */
{
  slint i, j;
  ...
  if (presorted && (rhigh < radix_high)) class_mask = ~(powof2_typed(rhigh + 1, slkey_pure_t) - 1);
...
slint pepcparts_rs_iter(elements_t *s, elements_t *sx, slint presorted, slint rhigh, slint rlow, slint rwidth) /* sl_func pepcparts_rs_iter */
{
#define max_nclasses (powof2_typed(sort_radix_width_max, slkey_pure_t))
  ...
  while (rhigh >= rlow)
  {
    ...
    while (xi.keys < end.keys)
    {
      ...
      do
      {
        i = radix_key2class(key_purify(*xi.keys), rhigh, bit_mask);
        ...
        else
      {
        ...
        pepcparts_rs_iter_insertsort(&xj, sx, presorted, rhigh_old, rlow);
      }
 #endif
#endif
    }
...
slint pepcparts_sort_radix_iter(elements_t *s, elements_t *sx, slint presorted, slint rhigh, slint rlow, slint rwidth) /* pepcparts_sl_proto, sl_func pepcparts_sort_radix_iter */
{
  elements_t _sx;
  ...
  pepcparts_rs_iter(s, sx, presorted, rhigh, rlow, xmin(rwidth, sort_radix_width_max));

#ifdef insertsort
 #ifdef insertsort_finalize
  if (sort_radix_threshold_iter > 1) pepcparts_rs_iter_insertsort(s, sx, presorted, rhigh, rlow);
 #endif
#endif
