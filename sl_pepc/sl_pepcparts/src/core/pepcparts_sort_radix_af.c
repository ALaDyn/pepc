 ...
 *  
 *  file: src/core/pepcparts_sort_radix_af.c
...
/* insert sort */
slint pepcparts_rs_rec_insertsort_af(elements_t *s, elements_t *sx, slint rhigh, slint rlow) /* sl_func pepcparts_rs_rec_insertsort_af */
{
  slint i, j;
...
slint pepcparts_rs_rec_af(elements_t *s, elements_t *sx, slint rhigh, slint rlow, slint rwidth, slint *finalize) /* sl_func pepcparts_rs_rec_af */
{
#define max_nclasses (powof2_typed(sort_radix_width_max, slkey_pure_t))
  ...
  if (rhigh >= rlow)
  {
    ...
    for (i = 0; i < nclasses; i++)
    {
...
#ifdef insertsort
      if (xi.size > sort_radix_threshold_rec) pepcparts_rs_rec_af(&xi, sx, rhigh, rlow, rwidth, finalize);
      ...
      else if (xi.size > 1) pepcparts_rs_rec_insertsort_af(&xi, sx, rhigh, rlow);
...
slint pepcparts_sort_radix_af(elements_t *s, elements_t *sx, slint rhigh, slint rlow, slint rwidth) /* pepcparts_sl_proto, sl_func pepcparts_sort_radix_af */
{
  elements_t _sx;
  ...
  pepcparts_rs_rec_af(s, sx, rhigh, rlow, xmin(rwidth, sort_radix_width_max), &finalize);

#ifdef insertsort_finalize
  if (sort_radix_threshold_rec > 1 && finalize) pepcparts_rs_rec_insertsort_af(s, sx, rhigh, rlow);
#endif /* insertsort_finalize */
