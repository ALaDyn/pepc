...
/* sl_func pepcparts_icrc1 pepcparts_icrc pepcparts_reflect pepcparts_crcInit pepcparts_crcFast */

/* Given a remainder up to now, return the new CRC after one character is added. This routine
is functionally equivalent to pepcparts_icrc(,,1,-1,1), but slower. It is used by pepcparts_icrc to initialize its
table. */
unsigned short pepcparts_icrc1(unsigned short crc, unsigned char onech)
{
  int i;
...
unsigned short pepcparts_icrc(unsigned short crc, unsigned char *bufptr, unsigned long len, short jinit, int jrev)
{
  unsigned short pepcparts_icrc1(unsigned short crc, unsigned char onech);
  ...
  if (!init) /* Do we need to initialize tables? */
  {
    ...
    for (j = 0; j <= 255; j++)
    {
      ...
      icrctb[j] = pepcparts_icrc1(j << 8, (unsigned char) 0);
...
unsigned short pepcparts_cs_crc16(elements_t *s, slint n, slint keys, slint data) /* pepcparts_sl_proto, sl_func pepcparts_cs_crc16 */
{
  ...
  {
    if (keys) crc += pepcparts_icrc(0, (unsigned char *) &s[j].keys[i], key_byte, 0, 0);

    if (data)
    {
#define xelem_key_not
#define xelem_call \
      crc += pepcparts_icrc(0, (unsigned char *) xelem_buf_at(&s[j], i), xelem_byte, 0, 1);
...
#undef  REFLECT_DATA
#define REFLECT_DATA(X)   ((unsigned char) pepcparts_reflect((X), 8))
#else
#undef  REFLECT_DATA
...
#define REFLECT_REMAINDER(X)  ((unsigned long) pepcparts_reflect((X), WIDTH))
#else
#undef  REFLECT_REMAINDER
#define REFLECT_REMAINDER(X)  (X)
#endif

static unsigned long pepcparts_reflect(unsigned long data, unsigned char nBits)
{
 ...
 *
 *********************************************************************/
void pepcparts_crcInit(unsigned long *crcTable)
{
...
/*********************************************************************
 *
 * Function:    pepcparts_crcFast()
 *
 * Description: Compute the CRC of a given message.
 *
 * Notes:   pepcparts_crcInit() must be called first.
 *
 * Returns:   The CRC of the message.
 *
 *********************************************************************/
unsigned long pepcparts_crcFast(unsigned char const message[], int nBytes, unsigned long *crcTable)
{
...
unsigned int pepcparts_cs_crc32(elements_t *s, slint n, slint keys, slint data) /* pepcparts_sl_proto, sl_func pepcparts_cs_crc32 */
{
  unsigned long crcTable[256];
  unsigned long crc = 0, crcp;
  slint i, j;

  pepcparts_crcInit(crcTable);
  ...
  for (j = 0; j < n; j++)
  {
    ...
    for (i = 0; i < s[j].size; i++)
    {
      if (keys) crcp += pepcparts_crcFast((unsigned char *) &s[j].keys[i], key_byte, crcTable);

      if (data)
      {
#define xelem_key_not
#define xelem_call \
        crcp += pepcparts_crcFast((unsigned char *) xelem_buf_at(&s[j], i), xelem_byte, crcTable);
#include "sl_xelem_call.h"
      }
    }
    crc += crcp;
