 ...
 *  
 *  file: src/core_mpi/pepcparts_mpi_find_exact.c
...
slint_t pepcparts_mpi_find_exact_equal(elements_t *s, slint_t other_rank, slint_t high_rank, slint_t *ex_start, slint_t *ex_size, int size, int rank, MPI_Comm comm) /* pepcparts_sl_proto, sl_func pepcparts_mpi_find_exact_equal */
{
  const int tag = 1;
...
#ifdef CHECK_EQUAL
  *my_cmp_i = s->size;
  MPI_Sendrecv(my_cmp_i, 1, sl_int_type_mpi, other_rank, tag, other_cmp_i, 1, sl_int_type_mpi, other_rank, tag, comm, &status);
  if (*other_cmp_i != s->size)
  {
    fprintf(stderr, "pepcparts_mpi_find_exact_equal: error: element lists not equal (%" sl_int_type_fmt " vs. %" sl_int_type_fmt ")\n", *my_cmp_i, *other_cmp_i);
    return -1;
  }
#endif
  ...
  while (interval.size > 0)
  {
    ...
    *my_cmp_key = key_get_pure(interval.keys[*my_cmp_i]);
...
slint_t pepcparts_mpi_find_exact(elements_t *s, slint_t other_rank, slint_t high_rank, slint_t *dst_size, slint_t *ex_start, slint_t *ex_sizes, slint_t *nx_move, int size, int rank, MPI_Comm comm) /* pepcparts_sl_proto, sl_func pepcparts_mpi_find_exact */
{
  const int tag = 1;
  ...
  MPI_Sendrecv(my_sizes, 2, sl_int_type_mpi, other_rank, tag, other_sizes, 2, sl_int_type_mpi, other_rank, tag, comm, &status);
  ...
  pepcparts_mpi_find_exact_equal(&interval, other_rank, high_rank, ex_start, ex_sizes, size, rank, comm);
