 ...
 *  
 *  file: src/core_mpi/pepcparts_mpi_splitk.c
...
/* Planloses isend/irecv kann zu deadlock führen WENN mpi-implementation bei wait[all|any] nicht alle requests prüft sondern nur EINEN
 ...
 Implementierung:
  ...
  -> "einfache" pepcparts_key2class_equal Funktion benötigt 8% von ewalk (bei 2^22 elements/node)
...
slint pepcparts_flow_create(slint *m, slint n, slint *complete, slint *reduced, int *send_counts, int *recv_counts, int rank) /* sl_func pepcparts_flow_create */
{
  slint i, j, k, y, z;
...
slint pepcparts_dfs_visit_rbe(slint v, slint maxflow, slint *pred, slint *m, slint n, slint *reduced, int *send_counts, int *recv_counts, int rank) /* sl_func pepcparts_dfs_visit_rbe */
{
  slint i, j, k;
...
/*  printf("visiting %d\n", v);*/
  ...
  if ((k = m[v * n + i]) > 0)
  {
    if (pred[i] == white) /* not visited, yet */
    {
      ...
      if (pepcparts_dfs_visit_rbe(i, xmin(maxflow, k), pred, m, n, reduced, send_counts, recv_counts, rank)) return 1;
...
slint pepcparts_cycle_reduce_back_edges(slint *m, slint n, slint *reduced, int *send_counts, int *recv_counts, int rank) /* sl_func pepcparts_cycle_reduce_back_edges */
{
...
/*  printf("reducing back edges\n");*/
  ...
  while (found_back_edge)
  {
    ...
    if (pred[i] == white)
    {
      ...
      if (pepcparts_dfs_visit_rbe(i, init_flow, pred, m, n, reduced, send_counts, recv_counts, rank))
      {
        found_back_edge = 1;
        break;
      }
...
slint pepcparts_flow_reduce_unbalanced(slint *m, slint n, slint *reduced, slint *max_unbalance, int *send_counts, int *recv_counts, int rank) /* sl_func pepcparts_flow_reduce_unbalanced */
{
  slint i, j, k;
...
slint pepcparts_mpi_splitk(elements_t *s, k2c_func k2c, void *ci, elements_t *sx, elements_t *sa, slint *nne, slint *nue, int size, int rank, MPI_Comm comm) /* pepcparts_sl_proto, sl_func pepcparts_mpi_splitk */
{
  slint i, j, k, l, t;

  slint unfinished, complete, reduced, remaining;
  ...
  while (1)
  {
    ...
    MPI_Allgather(local_sb_counts, size, sl_int_type_mpi, all_sb_counts, size, sl_int_type_mpi, comm);
    ...
    pepcparts_flow_create(all_sb_counts, size, &complete, &reduced, send_counts, recv_counts, rank);
    ...
    if (reduced < complete)
    {
      ...
      pepcparts_cycle_reduce_back_edges(all_sb_counts, size, &reduced, send_counts, recv_counts, rank);
    ...
    if (reduced < complete)
    {
      ...
      pepcparts_flow_reduce_unbalanced(all_sb_counts, size, &reduced, add_free_space, send_counts, recv_counts, rank);
