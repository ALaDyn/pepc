 ...
 *  
 *  file: src/core_mpi/pepcparts_mpi_partition_radix_old.c
 *  timestamp: 2010-01-04 09:51:26 +0100
 *  
 */
...
slint_t pepcparts_mpi_partition_radix_old(elements_t *s, partcond_t *pc, slint_t rhigh, slint_t rlow, slint_t rwidth, int *scounts, int *sdispls, int size, int rank, MPI_Comm comm) /* pepcparts_sl_proto, sl_func pepcparts_mpi_partition_radix_old */
{
...
# endif
  ...
  rti_treset(rti_tid_mpi_partition_radix_final);
...
/*    if (rank == 0)
    {
      printf("%d: pepcparts_mpi_partition_radix: allreduce: %f (nareas: %" sl_int_type_fmt ", nclasses: %" sl_key_type_fmt " -> %" sl_int_type_fmt " doubles)\n",
        rank,
        rti_tlast(rti_tid_mpi_partition_radix_while_allreduce),
        nareas,
        nclasses,
...
/*  printf("%d: nparts: %" sl_int_type_fmt "\n", rank, nparts);*/
  ...
  rti_tstart(rti_tid_mpi_partition_radix_final);
  ...
  rti_tstop(rti_tid_mpi_partition_radix_final);
...
/*      printf("%d: %" sl_int_type_fmt " %f / %f - %f / %f\n", rank, i, partial_weights[i], partial_weights[i] / partial_weights[size], (partial_weights[size] / size) - partial_weights[i], fabs(1.0 - (partial_weights[i] * size / partial_weights[size])));*/
  ...
  if (rank == 0)
  {
    printf("%d: pepcparts_mpi_partition_radix_old: %f\n", rank, rti_tlast(rti_tid_mpi_partition_radix));
    printf("%d: pepcparts_mpi_partition_radix_old: sync: %f\n", rank, rti_tlast(rti_tid_mpi_partition_radix_sync));
    printf("%d: pepcparts_mpi_partition_radix_old: while: %f\n", rank, rti_tlast(rti_tid_mpi_partition_radix_while));
    printf("%d: pepcparts_mpi_partition_radix_old:   count: %f\n", rank, rti_tcumu(rti_tid_mpi_partition_radix_while_count));
    printf("%d: pepcparts_mpi_partition_radix_old:   allreduce: %f\n", rank, rti_tcumu(rti_tid_mpi_partition_radix_while_allreduce));
    printf("%d: pepcparts_mpi_partition_radix_old:   round1: %f\n", rank, rti_tcumu(rti_tid_mpi_partition_radix_while_round1));
    printf("%d: pepcparts_mpi_partition_radix_old:     allgather: %f\n", rank, rti_tcumu(rti_tid_mpi_partition_radix_while_round1_allgather));
    printf("%d: pepcparts_mpi_partition_radix_old:   check: %f\n", rank, rti_tcumu(rti_tid_mpi_partition_radix_while_check));
    printf("%d: pepcparts_mpi_partition_radix_old: final: %f\n", rank, rti_tlast(rti_tid_mpi_partition_radix_final));
  }
#endif
