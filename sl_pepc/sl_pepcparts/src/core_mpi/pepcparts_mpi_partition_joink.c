 ...
 *  
 *  file: src/core_mpi/pepcparts_mpi_partition_joink.c
...
/* cycle nicht immer zu erst bei Prozess 0 suchen?
   ...
   - gegenüber pepcparts_mpi_splitk.c klare Aufteilung in local und Komm.
   - schneller?
...
slint pepcparts_dfs_visit_dcrbe(slint v, slint *max_flow, slint *pred, slint *m, slint n, slint *complete, slint *sendrecv_size, slint *send_rank, slint *recv_rank, int rank) /* sl_func pepcparts_dfs_visit_dcrbe */
{
  slint i, j, k, f;
  ...
  if ((k = m[v * n + i]) > 0)
  {
    if (pred[i] == white) /* not visited, yet */
    {
      ...
      /* visit */
      f = pepcparts_dfs_visit_dcrbe(i, max_flow, pred, m, n, complete, sendrecv_size, send_rank, recv_rank, rank);
...
slint pepcparts_disjoint_cycle_reduce_back_edges(slint *m, slint n, slint *complete, slint *sendrecv_size, slint *send_rank, slint *recv_rank, int rank) /* sl_func pepcparts_disjoint_cycle_reduce_back_edges */
{
  slint i, pred[n], flow;
  ...
  if (pred[i] == white)
  {
    ...
    pepcparts_dfs_visit_dcrbe(i, &flow, pred, m, n, complete, sendrecv_size, send_rank, recv_rank, rank);
...
slint pepcparts_mpi_partition_joink(elements_t *s, slint *sizes, int size, int rank, MPI_Comm comm) /* pepcparts_sl_proto, sl_func pepcparts_mpi_partition_joink */
{
  slint i, j, k, l, m, n;
...
/*  MPI_Barrier(comm);*/
  ...
  MPI_Allgather(local_sizes, size, sl_int_type_mpi, all_local_sizes, size, sl_int_type_mpi, comm);
...
/*    if (rank == 0)
    ...
    pepcparts_disjoint_cycle_reduce_back_edges(all_local_sizes, size, &complete, &sendrecv_size, &send_rank, &recv_rank, rank);
