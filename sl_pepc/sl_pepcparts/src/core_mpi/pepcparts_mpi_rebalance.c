 ...
 *  
 *  file: src/core_mpi/pepcparts_mpi_rebalance.c
...
slint_t pepcparts_mpi_rebalance(elements_t *s0, elements_t *s1, slint_t stable, slint_t *dst_size, int size, int rank, MPI_Comm comm) /* pepcparts_sl_proto, sl_func pepcparts_mpi_rebalance */
{
  const int tag = 1;
  int exit_code = 0;
  int i;
  ...
  {
    fprintf(stderr, "%d: pepcparts_mpi_rebalance: error: destination sizes too small (%" sl_int_type_fmt " > 0)\n", rank, s);
    exit_code = -1;
    goto free_and_exit;
  }
