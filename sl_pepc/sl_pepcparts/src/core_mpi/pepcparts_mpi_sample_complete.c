 ...
 *  
 *  file: src/core_mpi/pepcparts_mpi_sample_complete.c
...
slint pepcparts_mpi_sample_complete(elements_t *s, slint threshold, classification_info *ci, int size, int rank, MPI_Comm comm) /* pepcparts_sl_proto, sl_func pepcparts_mpi_sample_complete */
{
  const slint root = 0;
  ...
  MPI_Gatherv(s->keys, s->size, pepcparts_key_mpi_datatype, all_keys.keys, recv_counts, recv_displs, pepcparts_key_mpi_datatype, root, comm);
...
/*    pepcparts_elements_print_keys(&all_keys);*/
  }
  ...
  MPI_Bcast(ci->keys, size - 1, pepcparts_key_pure_mpi_datatype, root, comm);
