...
/*      if (rank == 0) printf("%d: %" sl_int_type_fmt ": current_borders: %f  %f  %f  %f\n", rank, parts[i], current_borders[0], current_borders[1], current_borders[2], current_borders[3]);*/
      ...
      Z_ASSERT(current_borders[0] <= current_borders[1]);
      Z_ASSERT(current_borders[2] >= current_borders[3]);
      Z_ASSERT(minmax_weights[(parts[i] + 1) * 4 + 2] <= minmax_weights[(parts[i] + 0) * 4 + 3]);

      current_minmax[0] = z_max3(current_borders[0], current_borders[3], minmax_weights[(parts[i] + 1) * 4 + 2]) - border_offsets[parts[i]];
      current_minmax[1] = z_min3(current_borders[2], current_borders[1], minmax_weights[(parts[i] + 0) * 4 + 3]) - border_offsets[parts[i]];

      Z_ASSERT(current_minmax[0] <= current_minmax[1]);
...
/*        gcs += gc;*/
        ...
        if (current_minmax[0] <= 0 && current_minmax[1] >= 0)
        {
          ...
          if (z_abs(current_minmax[0] + current_minmax[1]) > z_abs(current_minmax[0] + current_minmax[1] - 2 * global_weights[part_areas[i] * nclasses + k + 1])) continue;
...
/*        if (rank == 0)
        ...
        } else
        {
          ...
          Z_ASSERT(bsets[bsets_parent[parts[i]]] == j);
        }
        ++nbsets;
...
/*  printf("%d: nparts: %" sl_int_type_fmt "\n", rank, nparts);*/
  ...
  if (nparts > 0)
  {
    ...
    for (i = 0; i < nparts; ++i)
    {
      d = z_max(0, border_finalize[parts[i]] - local_weights[i]);
