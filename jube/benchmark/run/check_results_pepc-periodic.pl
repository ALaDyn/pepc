...
print "compare fields in $ARGV[1] and $ARGV[2]\n";
...
# skip further 3 lines in both files
readline(file1);
readline(file1);
readline(file1);
readline(file2);
...
while( defined ($line1 = readline(file1)) && defined ($line2 = readline(file2)) )
{
    ...
    print "linediff: $diffp $diffx $diffy $diffz $diffvx $diffvy\n";
    print "\n";

    $L2norm = $L2norm + sqrt($diffp*$diffp + 
			     $diffx*$diffx + $diffy*$diffy + $diffz*$diffz + 
			     $diffvx*$diffvx + $diffvy*$diffvy );
