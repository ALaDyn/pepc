...
subroutine forces(p_start,p_finish,delta_t, t_domain,t_build,t_prefetch, t_walk, t_walkc, t_force)
  ...
  load_average = SUM(work(1:np_local))/npass   ! Ave. workload per pass: same for all PEs if already load balanced
  nshort(1:npass+1) = 0
  ...
  do i=1,np_local
     ...
     else if (load_integral >= load_average * jpass .or. i==np_local ) then
  ...
  call MPI_GATHER(np_local, 1, MPI_INTEGER, npps, 1, MPI_INTEGER, 0,  MPI_COMM_WORLD, ierr )  ! Gather particle distn
