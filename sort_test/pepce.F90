...
program pepce
  ...
  ! timing stuff
  real :: t0, t_key, t_domain=0., t_build=0., t_branches=0., t_fill=0., t_properties=0., t_restore=0.
  real :: t_begin=0.,t_walk=0., t_walkc=0., t_en, t_force=0.
  ...
  integer :: ierr, lvisit_active, ifile, debug, i
...
!  allocate(indxl(nppm),irnkl(nppm),islen(n_cpu),irlen(n_cpu),fposts(n_cpu+1),gposts(n_cpu+1))
...
!                 t_domain,t_build,t_prefetch,t_walk,t_walkc,t_force,t_restore)   ! Compute Coulomb fields and pot using lpepc

!     call cputime(t_restore)
...
!     t_restore = t_start_push - t_restore    
