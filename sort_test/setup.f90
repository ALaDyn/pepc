...
subroutine setup
  ...
  namelist /sortdata/ nep, nip, np_mult, fetch_mult, ne, ni, &
  ...
  open(10,file='run_sort.h')
  read (10,NML=sortdata)
...
!     write(*,*) '-= Setup =-'
!     write(*,*) 'memory consumption = ',(15*size(x)*8+2*size(vbuffer)*4)/1024/1024,'MiB'
!  end if

  if (my_rank == 0) write(*,*) "Starting Sort-Test with",n_cpu,"Cores, sorting",np_local,"Particles on each Core..."
