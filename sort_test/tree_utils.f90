...
! unweighted || sort
  ...
  subroutine psrssort(nppm,np,npnew,nprocs,iproc,keys,indxl,irnkl,islen,irlen,fposts,gposts,w1)
    ...
    integer :: nppm,np,npnew,iproc,nprocs
...
!    integer, parameter :: maxprocs = 1024
    integer*8 :: itabr(nprocs), itabl(nprocs+1)

    integer*8 :: work(nprocs*(nprocs+1))
    integer*8 :: fpval(nprocs+1)
    integer*8 :: lmax, ak

    integer*8 :: buf(nprocs)
...
!          if (iproc == 0) write(*,*) w1(i),fpval(k), w1(i) .ge. fpval(k)
          fposts(k) = i
          k = k + 1
       enddo
    enddo
...
!  ===================================================================================================
  ...
  subroutine pbalsort(nppm,np,npnew,nprocs,iproc,keys, &
    ...
    integer, intent(in) :: nppm,np,iproc,nprocs
...
!    integer*8, parameter :: iplace=0
    ...
    integer*8 :: ilev, p, np_left, new_bins, nfbins
    character(13) :: cfmt
    integer, save :: icall
    data icall/0/
...
!    key_max = key_box(2)
    ...
    do ibin = 1,nbin
	  ...
	  if (nfill>0) then 
	        write (*,'(a20,i8/a50,i8/a10,f12.3,a10,f12.3)') 'Problem on Processor ',iproc &
...
!	       call closefiles
  ...
  subroutine nwaymrg(nppm,np,nprocs,keys,irnkl,itabl,itabr,me)
    ...
    integer :: irnkl(nppm)
    integer*8 :: keys(nppm)
    integer*8 :: itabl(nprocs+1),itabr(nprocs)

!    integer, parameter :: maxprocs=nprocs
