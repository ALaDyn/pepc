...
print    "<tr><td width=\"110\">Name$marker:</td>\n<td width=\"275\">";
print     $query->textfield(-name => 'fullname', -size => 80),"</td></tr>\n";

print    "<tr><td width=\"110\">Institution:</td>\n<td width=\"275\">";
...
#	    print $query->br;
	    print $query->b('Version 1.4 (May 2006):');
	    
	    print $query->ul(
			     $query->li("Source tar file ",
					$query->a({href=>"$tmpurl/pepc1.4.tar.gz"},
						  'pepc1.4.tar.gz')
					)
			     );
	    print $query->b('Version 1.7 (last update: 24/02/2007):');
	    print $query->ul(
			     $query->li("Source tar file ",
					$query->a({href=>"$tmpurl/pepc1.7.tar.gz"},
						  'pepc1.7.tar.gz')
					)
			     );
	    ...
	    print $query->ul(
			     $query->li("Demo tar file",
					$query->a({href=>"$tmpurl/pepc_demos1.1.tar.gz"},
						  'pepc_demos1.1.tar.gz')
					)
			     );
...
#	print "<BR>--mkdir $tmppath ($?)<BR>";
	...
	unlink("$tmppath/pepc1.4.tar.gz");
	unlink("$tmppath/pepc1.7.tar.gz");
	unlink("$tmppath/pepc_demos1.1.tar.gz");
    }
    symlink("$softwarepath/pepc1.4.tar.gz","$tmppath/pepc1.4.tar.gz");
    symlink("$softwarepath/pepc1.7.tar.gz","$tmppath/pepc1.7.tar.gz");
    symlink("$softwarepath/pepc_demos1.1.tar.gz","$tmppath/pepc_demos1.1.tar.gz");
